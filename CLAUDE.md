...
- **Ship model — trunk + two chat-driven gates (2026-07-13; the trunk branch was renamed `main`→`trunk` on 2026-07-13).** `trunk` is the **trunk branch**:
  ...
  - Big *replacements* ship behind a `FEATURES` flag in `config.js` (`flagOn()` reader) so
    backing a swap out is a toggle, not code surgery (a flag hides execution, not source, on
    public Pages).
...
- **Cache-bust on every deploy:** bump the `?v=` token on `style.css`, `rule-usage.js`,
  ...
  (`tools/deploy-staging.mjs` performs this shared-token bump for you on a staging deploy.)
- **Staging is a direct push now (2026-07-13 — replaces the old cron mirror).**
  `tools/deploy-staging.mjs` clones the staging repo (`operations-jacrentals/rental-wrangler-staging`,
  Pages branch `main`), syncs the crawled site-file set, bumps the shared `?v=`, and pushes;
  the staging URL updates within ~1 min. There is no longer a `sync-staging.yml` cron/dispatch,
  and no more "mirror serves an OLD file under a NEW token" trap. Verify the new bytes:
  `curl -s https://operations-jacrentals.github.io/rental-wrangler-staging/index.html | grep <new ?v=>`.
  The cross-repo push authenticates with the `STAGING_DEPLOY_PAT` env secret — never echo or hardcode it.
