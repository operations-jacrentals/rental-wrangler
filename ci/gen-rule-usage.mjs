...
// globeToggle takes a dynamic card id + on-state, not a literal label — presence-check like fileDrop.
if (/globeToggle\(/.test(src)) add('R33', 'card-search global-mode globe (all grid-card bars)');
// swipeTrack/swipeSeg build the phone swipe-toggle decks — presence-check (dynamic panes, no literal label).
if (/swipeTrack\(/.test(src)) add('R36', 'phone swipe-toggle decks (funnel · invoices · comms Text/Email)');
