...
try {
  ...
  const results = await page.evaluate(async () => {
    ...
    // with the real gates (fleet-Active, blacklist, overbooking, valid window). Priced by the engine.
    {
      ...
      // The current account UI stores a manual blacklist in c.block; every rental-entry path
      // must recognize it alongside the legacy accountType value.
      const storedBlacklisted = { customerId: 'C-BLOCK', name: 'Stored Block Co', accountType: 'Non-Business', block: { type: 'blacklist', setBy: 'Admin', setAt: T.TODAY_ISO }, firstName: '', lastName: '', activityLog: [] };
      T.DATA.customers.push(storedBlacklisted); T.IDX.customer.set(storedBlacklisted.customerId, storedBlacklisted);
      ok(wr({ customerId: storedBlacklisted.customerId, unitIds: [freeUnit.unitId], startDate: '2099-11-01', endDate: '2099-11-08' }).issues.some((s) => /blacklisted/.test(s)), 'WR-rent: a stored c.block blacklist is refused');
      const directQuoteIdsBefore = new Set(T.DATA.rentals.map((r) => r.rentalId));
      window.JT.startNewRental(storedBlacklisted.customerId);
      const directQuoteLeaks = T.DATA.rentals.filter((r) => !directQuoteIdsBefore.has(r.rentalId));
      ok(directQuoteLeaks.length === 0, 'quote creation gate: +New Rental cannot create a prospective Quote for a stored c.block blacklist');
      directQuoteLeaks.forEach((r) => { const i = T.DATA.rentals.indexOf(r); if (i >= 0) T.DATA.rentals.splice(i, 1); T.IDX.rental.delete(r.rentalId); });   // failed-before cleanup
      const linkProbe = { rentalId: 'R-BL-LINK', customerId: 'C0009', rentalName: 'Blacklist link probe', status: 'Reserved', units: [], actions: [] };
      T.DATA.rentals.push(linkProbe); T.IDX.rental.set(linkProbe.rentalId, linkProbe);
      ok(T.DROP_MATRIX.rentals.customers(linkProbe, storedBlacklisted) === false && T.DROP_MATRIX.customers.rentals(storedBlacklisted, linkProbe) === false, 'link gate: stored c.block blacklist never lights as a valid rental drop');
      const linkResult = window.JT.linkCustomerToRental(linkProbe.rentalId, storedBlacklisted.customerId);
      ok(linkResult === null && linkProbe.customerId === 'C0009', 'link gate: stored c.block blacklist cannot be attached to an existing rental');
      const linkProbeIdx = T.DATA.rentals.indexOf(linkProbe); if (linkProbeIdx >= 0) T.DATA.rentals.splice(linkProbeIdx, 1); T.IDX.rental.delete(linkProbe.rentalId);
      const expansionProbe = { rentalId: 'R-BL-EXPAND', customerId: storedBlacklisted.customerId, rentalName: 'Blacklist expansion probe', startDate: '2099-12-10', endDate: '2099-12-12', status: 'Reserved', units: [], actions: [] };
      T.DATA.rentals.push(expansionProbe); T.IDX.rental.set(expansionProbe.rentalId, expansionProbe);
      ok(T.DROP_MATRIX.units.rentals(freeUnit, expansionProbe) === false && T.DROP_MATRIX.rentals.units(expansionProbe, freeUnit) === false, 'unit expansion gate: a stored c.block blacklist never lights as a valid unit↔rental drop');
      const expansionResult = window.JT.linkUnitToRental(expansionProbe.rentalId, freeUnit.unitId);
      ok(expansionResult === null && expansionProbe.units.length === 0, 'unit expansion gate: a blacklisted rental cannot gain newly reserved equipment');
      const expansionProbeIdx = T.DATA.rentals.indexOf(expansionProbe); if (expansionProbeIdx >= 0) T.DATA.rentals.splice(expansionProbeIdx, 1); T.IDX.rental.delete(expansionProbe.rentalId);
      const invoiceOwnedBlacklist = { invoiceId: 'I-BL-OWNER', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [{ kind: 'custom', lid: 'L-BL-OWNER', label: 'Blacklist ownership probe', amount: 10 }] };
      const customerlessProbe = { rentalId: 'R-BL-CUSTOMERLESS', customerId: null, invoiceId: invoiceOwnedBlacklist.invoiceId, rentalName: 'Customerless blacklist probe', startDate: '2099-12-14', endDate: '2099-12-16', status: 'Quote', units: [], actions: [] };
      T.DATA.invoices.push(invoiceOwnedBlacklist); T.IDX.invoice.set(invoiceOwnedBlacklist.invoiceId, invoiceOwnedBlacklist);
      T.DATA.rentals.push(customerlessProbe); T.IDX.rental.set(customerlessProbe.rentalId, customerlessProbe);
      ok(T.DROP_MATRIX.units.rentals(freeUnit, customerlessProbe) === false && T.DROP_MATRIX.rentals.units(customerlessProbe, freeUnit) === false, 'rental ownership fallback: a customerless rental still honors its blacklisted invoice customer in drag eligibility');
      const customerlessLink = window.JT.linkUnitToRental(customerlessProbe.rentalId, freeUnit.unitId);
      ok(customerlessLink === null && customerlessProbe.units.length === 0, 'rental ownership fallback: a customerless rental cannot bypass blacklist through hard unit expansion');
      window.JT.setRentalStatus(customerlessProbe.rentalId, 'End Rent');
      ok(customerlessProbe.status === 'Quote', 'rental ownership fallback: a customerless rental cannot bypass blacklist through a status shortcut');
      const customerlessProbeIdx = T.DATA.rentals.indexOf(customerlessProbe); if (customerlessProbeIdx >= 0) T.DATA.rentals.splice(customerlessProbeIdx, 1); T.IDX.rental.delete(customerlessProbe.rentalId);
      const invoiceOwnedIdx = T.DATA.invoices.indexOf(invoiceOwnedBlacklist); if (invoiceOwnedIdx >= 0) T.DATA.invoices.splice(invoiceOwnedIdx, 1); T.IDX.invoice.delete(invoiceOwnedBlacklist.invoiceId);
      const swapTarget = T.IDX.customer.get('C0009');
      const swapProbe = { rentalId: 'R-BL-SWAP', customerId: storedBlacklisted.customerId, rentalName: 'Blacklist customer swap probe', status: 'Quote', units: [], actions: [] };
      T.DATA.rentals.push(swapProbe); T.IDX.rental.set(swapProbe.rentalId, swapProbe);
      ok(T.DROP_MATRIX.rentals.customers(swapProbe, swapTarget) === false && T.DROP_MATRIX.customers.rentals(swapTarget, swapProbe) === false, 'rental customer freeze: a blacklisted rental never lights as a customer-reassignment drop');
      const swapResult = window.JT.linkCustomerToRental(swapProbe.rentalId, swapTarget.customerId);
      ok(swapResult === null && swapProbe.customerId === storedBlacklisted.customerId, 'rental customer freeze: the hard reassignment mutation cannot erase the current blacklist');
      const clearSwapResult = T.clearRentalCustomer(swapProbe);
      ok(clearSwapResult === false && swapProbe.customerId === storedBlacklisted.customerId, 'rental customer freeze: the hard removal mutation cannot erase the current blacklist');
      if (swapProbe.customerId !== storedBlacklisted.customerId) swapProbe.customerId = storedBlacklisted.customerId;   // failed-before cleanup
      const swapProbeIdx = T.DATA.rentals.indexOf(swapProbe); if (swapProbeIdx >= 0) T.DATA.rentals.splice(swapProbeIdx, 1); T.IDX.rental.delete(swapProbe.rentalId);
      const attachInvoice = { invoiceId: 'I-BL-ATTACH', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const attachProbe = { rentalId: 'R-BL-ATTACH', customerId: null, invoiceId: null, rentalName: 'Blacklist invoice attach probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'Quote', units: [{ unitId: freeUnit.unitId, status: 'Quote' }], actions: [] };
      T.DATA.invoices.push(attachInvoice); T.IDX.invoice.set(attachInvoice.invoiceId, attachInvoice);
      T.DATA.rentals.push(attachProbe); T.IDX.rental.set(attachProbe.rentalId, attachProbe);
      ok(T.DROP_MATRIX.rentals.invoices(attachProbe, attachInvoice) === false && T.DROP_MATRIX.invoices.rentals(attachInvoice, attachProbe) === false, 'invoice ownership gate: a customerless rental cannot attach to a blacklisted customer invoice');
      window.JT.addRentalLineToInvoice(attachInvoice.invoiceId, attachProbe.rentalId);
      ok(!attachProbe.invoiceId && attachInvoice.rentalIds.length === 0 && attachInvoice.lineItems.length === 0, 'invoice ownership gate: the hard billing mutation refuses a customerless rental under a blacklisted invoice');
      const attachProbeIdx = T.DATA.rentals.indexOf(attachProbe); if (attachProbeIdx >= 0) T.DATA.rentals.splice(attachProbeIdx, 1); T.IDX.rental.delete(attachProbe.rentalId);
      const attachInvoiceIdx = T.DATA.invoices.indexOf(attachInvoice); if (attachInvoiceIdx >= 0) T.DATA.invoices.splice(attachInvoiceIdx, 1); T.IDX.invoice.delete(attachInvoice.invoiceId);
      const directProspectiveProbe = { rentalId: 'R-BL-DIRECT-INVOICE', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Blacklist direct invoice probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'Reserved', units: [{ unitId: freeUnit.unitId, status: 'Reserved' }], actions: [], mock: true };
      T.DATA.rentals.push(directProspectiveProbe); T.IDX.rental.set(directProspectiveProbe.rentalId, directProspectiveProbe);
      const directProspectiveInvoiceCount = T.DATA.invoices.length;
      T.createInvoiceForRental(directProspectiveProbe.rentalId);
      ok(!directProspectiveProbe.invoiceId && T.DATA.invoices.length === directProspectiveInvoiceCount, 'invoice ownership gate: +Invoice cannot bill a prospective Reserved rental after its customer is blacklisted');
      T.rentalInvoices(directProspectiveProbe).forEach((inv) => { const i = T.DATA.invoices.indexOf(inv); if (i >= 0) T.DATA.invoices.splice(i, 1); T.IDX.invoice.delete(inv.invoiceId); });
      const directProspectiveProbeIdx = T.DATA.rentals.indexOf(directProspectiveProbe); if (directProspectiveProbeIdx >= 0) T.DATA.rentals.splice(directProspectiveProbeIdx, 1); T.IDX.rental.delete(directProspectiveProbe.rentalId);
      const outInvoice = { invoiceId: 'I-BL-OUT-ATTACH', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const outProbe = { rentalId: 'R-BL-OUT-ATTACH', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Already-out blacklist billing probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }], actions: [] };
      T.DATA.invoices.push(outInvoice); T.IDX.invoice.set(outInvoice.invoiceId, outInvoice);
      T.DATA.rentals.push(outProbe); T.IDX.rental.set(outProbe.rentalId, outProbe);
      ok(T.DROP_MATRIX.rentals.invoices(outProbe, outInvoice) === true && T.DROP_MATRIX.invoices.rentals(outInvoice, outProbe) === true, 'invoice ownership gate: blacklisting does not strand already-out equipment; matching On Rent billing remains droppable');
      window.JT.addRentalLineToInvoice(outInvoice.invoiceId, outProbe.rentalId);
      ok(outProbe.invoiceId === outInvoice.invoiceId && outInvoice.rentalIds.includes(outProbe.rentalId) && outInvoice.lineItems.some((li) => li.kind === 'rental' && li.ref === outProbe.rentalId), 'invoice ownership gate: the hard billing mutation still attaches an already-out blacklisted rental for recovery/return accounting');
      const outProbeIdx = T.DATA.rentals.indexOf(outProbe); if (outProbeIdx >= 0) T.DATA.rentals.splice(outProbeIdx, 1); T.IDX.rental.delete(outProbe.rentalId);
      const outInvoiceIdx = T.DATA.invoices.indexOf(outInvoice); if (outInvoiceIdx >= 0) T.DATA.invoices.splice(outInvoiceIdx, 1); T.IDX.invoice.delete(outInvoice.invoiceId);
      const directOutProbe = { rentalId: 'R-BL-DIRECT-OUT-INVOICE', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Already-out direct invoice probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }], actions: [], mock: true };
      T.DATA.rentals.push(directOutProbe); T.IDX.rental.set(directOutProbe.rentalId, directOutProbe);
      const directOutInvoiceCount = T.DATA.invoices.length;
      T.createInvoiceForRental(directOutProbe.rentalId);
      const directOutInvoices = T.rentalInvoices(directOutProbe);
      ok(!!directOutProbe.invoiceId && T.DATA.invoices.length > directOutInvoiceCount && directOutInvoices.some((inv) => inv.lineItems.some((li) => li.kind === 'rental' && li.ref === directOutProbe.rentalId)), 'invoice ownership gate: +Invoice remains available for already-out blacklisted recovery billing');
      directOutInvoices.forEach((inv) => { const i = T.DATA.invoices.indexOf(inv); if (i >= 0) T.DATA.invoices.splice(i, 1); T.IDX.invoice.delete(inv.invoiceId); });
      const directOutProbeIdx = T.DATA.rentals.indexOf(directOutProbe); if (directOutProbeIdx >= 0) T.DATA.rentals.splice(directOutProbeIdx, 1); T.IDX.rental.delete(directOutProbe.rentalId);
      const mixedTerminalUnit = T.DATA.units.find((u) => u.unitId !== freeUnit.unitId);
      const mixedTerminalInvoice = { invoiceId: 'I-BL-OUT-MIXED', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const mixedTerminalProbe = { rentalId: 'R-BL-OUT-MIXED', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Already-out plus voided sibling', startDate: '2099-12-20', endDate: '2099-12-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }, { unitId: mixedTerminalUnit.unitId, status: 'Cancelled' }], actions: [] };
      T.DATA.invoices.push(mixedTerminalInvoice); T.IDX.invoice.set(mixedTerminalInvoice.invoiceId, mixedTerminalInvoice);
      T.DATA.rentals.push(mixedTerminalProbe); T.IDX.rental.set(mixedTerminalProbe.rentalId, mixedTerminalProbe);
      ok(T.DROP_MATRIX.rentals.invoices(mixedTerminalProbe, mixedTerminalInvoice) === true && T.DROP_MATRIX.invoices.rentals(mixedTerminalInvoice, mixedTerminalProbe) === true, 'invoice recovery gate: a voided sibling does not strand billing for equipment already On Rent');
      window.JT.addRentalLineToInvoice(mixedTerminalInvoice.invoiceId, mixedTerminalProbe.rentalId);
      const mixedRentalLines = mixedTerminalInvoice.lineItems.filter((li) => li.kind === 'rental' && li.ref === mixedTerminalProbe.rentalId);
      ok(mixedTerminalProbe.invoiceId === mixedTerminalInvoice.invoiceId && mixedRentalLines.length === 1 && mixedRentalLines[0].unitId === freeUnit.unitId, 'invoice recovery gate: hard billing attaches the mixed rental and excludes its Cancelled sibling');
      const mixedTerminalProbeIdx = T.DATA.rentals.indexOf(mixedTerminalProbe); if (mixedTerminalProbeIdx >= 0) T.DATA.rentals.splice(mixedTerminalProbeIdx, 1); T.IDX.rental.delete(mixedTerminalProbe.rentalId);
      const mixedTerminalInvoiceIdx = T.DATA.invoices.indexOf(mixedTerminalInvoice); if (mixedTerminalInvoiceIdx >= 0) T.DATA.invoices.splice(mixedTerminalInvoiceIdx, 1); T.IDX.invoice.delete(mixedTerminalInvoice.invoiceId);
      const mixedProspectiveInvoice = { invoiceId: 'I-BL-OUT-PROSPECTIVE', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const mixedProspectiveProbe = { rentalId: 'R-BL-OUT-PROSPECTIVE', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Already-out plus reserved sibling', startDate: '2099-12-20', endDate: '2099-12-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }, { unitId: mixedTerminalUnit.unitId, status: 'Reserved' }], actions: [] };
      T.DATA.invoices.push(mixedProspectiveInvoice); T.IDX.invoice.set(mixedProspectiveInvoice.invoiceId, mixedProspectiveInvoice);
      T.DATA.rentals.push(mixedProspectiveProbe); T.IDX.rental.set(mixedProspectiveProbe.rentalId, mixedProspectiveProbe);
      ok(T.DROP_MATRIX.rentals.invoices(mixedProspectiveProbe, mixedProspectiveInvoice) === false && T.DROP_MATRIX.invoices.rentals(mixedProspectiveInvoice, mixedProspectiveProbe) === false, 'invoice recovery gate: an On Rent sibling does not hide a still-Reserved blacklisted allocation');
      window.JT.addRentalLineToInvoice(mixedProspectiveInvoice.invoiceId, mixedProspectiveProbe.rentalId);
      ok(!mixedProspectiveProbe.invoiceId && mixedProspectiveInvoice.rentalIds.length === 0 && mixedProspectiveInvoice.lineItems.length === 0, 'invoice recovery gate: hard billing still refuses a mixed rental with prospective Reserved equipment');
      const mixedProspectiveProbeIdx = T.DATA.rentals.indexOf(mixedProspectiveProbe); if (mixedProspectiveProbeIdx >= 0) T.DATA.rentals.splice(mixedProspectiveProbeIdx, 1); T.IDX.rental.delete(mixedProspectiveProbe.rentalId);
      const mixedProspectiveInvoiceIdx = T.DATA.invoices.indexOf(mixedProspectiveInvoice); if (mixedProspectiveInvoiceIdx >= 0) T.DATA.invoices.splice(mixedProspectiveInvoiceIdx, 1); T.IDX.invoice.delete(mixedProspectiveInvoice.invoiceId);
      const allVoidedInvoice = { invoiceId: 'I-BL-ALL-VOIDED', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const allVoidedProbe = { rentalId: 'R-BL-ALL-VOIDED', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'All voided blacklist probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'Cancelled', units: [{ unitId: freeUnit.unitId, status: 'Cancelled' }, { unitId: mixedTerminalUnit.unitId, status: 'No Show' }], actions: [] };
      T.DATA.invoices.push(allVoidedInvoice); T.IDX.invoice.set(allVoidedInvoice.invoiceId, allVoidedInvoice);
      T.DATA.rentals.push(allVoidedProbe); T.IDX.rental.set(allVoidedProbe.rentalId, allVoidedProbe);
      window.JT.addRentalLineToInvoice(allVoidedInvoice.invoiceId, allVoidedProbe.rentalId);
      ok(!allVoidedProbe.invoiceId && allVoidedInvoice.rentalIds.length === 0 && allVoidedInvoice.lineItems.length === 0, 'invoice recovery gate: an all-voided blacklisted rental cannot create an empty invoice attachment');
      const allVoidedProbeIdx = T.DATA.rentals.indexOf(allVoidedProbe); if (allVoidedProbeIdx >= 0) T.DATA.rentals.splice(allVoidedProbeIdx, 1); T.IDX.rental.delete(allVoidedProbe.rentalId);
      const allVoidedInvoiceIdx = T.DATA.invoices.indexOf(allVoidedInvoice); if (allVoidedInvoiceIdx >= 0) T.DATA.invoices.splice(allVoidedInvoiceIdx, 1); T.IDX.invoice.delete(allVoidedInvoice.invoiceId);
      const zeroUnitInvoice = { invoiceId: 'I-BL-ZERO-UNIT', customerId: storedBlacklisted.customerId, amountPaid: 0, lineItems: [], rentalIds: [], mock: true };
      const zeroUnitProbe = { rentalId: 'R-BL-ZERO-UNIT', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Zero-unit blacklist probe', startDate: '2099-12-20', endDate: '2099-12-22', status: 'On Rent', units: [], actions: [] };
      T.DATA.invoices.push(zeroUnitInvoice); T.IDX.invoice.set(zeroUnitInvoice.invoiceId, zeroUnitInvoice);
      T.DATA.rentals.push(zeroUnitProbe); T.IDX.rental.set(zeroUnitProbe.rentalId, zeroUnitProbe);
      window.JT.addRentalLineToInvoice(zeroUnitInvoice.invoiceId, zeroUnitProbe.rentalId);
      ok(!zeroUnitProbe.invoiceId && zeroUnitInvoice.rentalIds.length === 0 && zeroUnitInvoice.lineItems.length === 0, 'invoice recovery gate: a zero-unit blacklisted record cannot create an empty invoice attachment from a stale On Rent master status');
      const zeroUnitProbeIdx = T.DATA.rentals.indexOf(zeroUnitProbe); if (zeroUnitProbeIdx >= 0) T.DATA.rentals.splice(zeroUnitProbeIdx, 1); T.IDX.rental.delete(zeroUnitProbe.rentalId);
      const zeroUnitInvoiceIdx = T.DATA.invoices.indexOf(zeroUnitInvoice); if (zeroUnitInvoiceIdx >= 0) T.DATA.invoices.splice(zeroUnitInvoiceIdx, 1); T.IDX.invoice.delete(zeroUnitInvoice.invoiceId);
      const quoteProbe = { rentalId: 'R-BL-QUOTE', customerId: storedBlacklisted.customerId, rentalName: 'Blacklist quote probe', startDate: '', endDate: '', startTime: '', status: 'Quote', units: [], actions: [] };
      T.DATA.rentals.push(quoteProbe); T.IDX.rental.set(quoteProbe.rentalId, quoteProbe);
      window.JT.setDraftDate(quoteProbe.rentalId, 'start', '2099-10-01'); window.JT.setDraftDate(quoteProbe.rentalId, 'end', '2099-10-03');
      ok(quoteProbe.status === 'Quote', 'human rent: dating a Quote for a stored c.block blacklist cannot turn it into Reserved');
      quoteProbe.startDate = ''; quoteProbe.endDate = '';
      T.__state.winEdit = { rentalId: quoteProbe.rentalId, monthISO: '2099-10-01', anchor: null };
      T.winPickDay('2099-10-07'); T.winPickDay('2099-10-09');
      ok(quoteProbe.status === 'Quote', 'human rent: inline calendar cannot turn a stored c.block blacklist Quote into Reserved');
      quoteProbe.startDate = ''; quoteProbe.endDate = '';
      T.__state.winEdit = { rentalId: quoteProbe.rentalId, monthISO: '2099-10-01', anchor: null, staged: { rentalId: quoteProbe.rentalId, startDate: '2099-10-11', endDate: '2099-10-13', startTime: '' } };
      T.winPickSave();
      ok(quoteProbe.status === 'Quote', 'human rent: staged calendar save cannot turn a stored c.block blacklist Quote into Reserved');
      const quoteProbeIdx = T.DATA.rentals.indexOf(quoteProbe); if (quoteProbeIdx >= 0) T.DATA.rentals.splice(quoteProbeIdx, 1); T.IDX.rental.delete(quoteProbe.rentalId);
      const statusProbe = { rentalId: 'R-BL-STATUS', customerId: storedBlacklisted.customerId, rentalName: 'Blacklist status probe', startDate: '2099-10-15', endDate: '2099-10-17', status: 'Quote', units: [{ unitId: freeUnit.unitId, status: 'Quote' }], actions: [] };
      T.DATA.rentals.push(statusProbe); T.IDX.rental.set(statusProbe.rentalId, statusProbe);
      window.JT.setRentalStatus(statusProbe.rentalId, 'End Rent');
      ok(statusProbe.status === 'Quote' && statusProbe.units[0].status === 'Quote', 'status gate: a stored c.block blacklist cannot jump a Quote master directly to End Rent');
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'Off Rent');
      ok(statusProbe.status === 'Quote' && statusProbe.units[0].status === 'Quote', 'status gate: a stored c.block blacklist cannot jump a Quote unit directly to Off Rent');
      window.JT.setRentalStatus(statusProbe.rentalId, 'Returned');
      ok(statusProbe.status === 'Quote' && statusProbe.units[0].status === 'Quote', 'status gate: a prospective blacklisted rental cannot jump from Quote directly to Returned and manufacture physical/billing history');
      statusProbe.status = 'Quote'; statusProbe.units[0].status = 'Quote';
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'Returned');
      ok(statusProbe.status === 'Quote' && statusProbe.units[0].status === 'Quote', 'status gate: a prospective blacklisted unit cannot jump from Quote directly to Returned and bypass acquisition');
      statusProbe.status = 'Quote'; statusProbe.units[0].status = 'Quote';
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setRentalStatus(statusProbe.rentalId, 'Quote');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted rental cannot move backward to Quote and hide its allocation');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'Quote');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted unit cannot move backward to Quote and appear available');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setRentalStatus(statusProbe.rentalId, 'Cancelled');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted rental cannot bypass recovery by moving to Cancelled');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setRentalStatus(statusProbe.rentalId, 'No Show');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted rental cannot bypass recovery by moving to No Show');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'Cancelled');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted unit cannot bypass recovery by moving to Cancelled');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'No Show');
      ok(statusProbe.status === 'On Rent' && statusProbe.units[0].status === 'On Rent', 'status gate: an already-out blacklisted unit cannot bypass recovery by moving to No Show');
      statusProbe.status = 'On Rent'; statusProbe.units[0].status = 'On Rent';
      window.JT.setRentalStatus(statusProbe.rentalId, 'End Rent');
      ok(statusProbe.status === 'End Rent' && statusProbe.units[0].status === 'End Rent', 'status gate: blacklisting does not strand an already-out rental; On Rent can progress to End Rent');
      window.JT.setUnitStatus(statusProbe.rentalId, freeUnit.unitId, 'Off Rent');
      ok(statusProbe.status === 'Off Rent' && statusProbe.units[0].status === 'Off Rent', 'status gate: blacklisting does not strand recovery; an already-out unit can progress to Off Rent');
      const returnedHistoryInvoice = { invoiceId: 'I-BL-RETURNED-HISTORY', customerId: storedBlacklisted.customerId, rentalIds: ['R-BL-RETURNED-HISTORY'], amountPaid: 0, lineItems: [
        { kind: 'rental', ref: 'R-BL-RETURNED-HISTORY', unitId: freeUnit.unitId, lid: 'L-BL-RETURNED-A', label: freeUnit.name, amount: 10 },
        { kind: 'rental', ref: 'R-BL-RETURNED-HISTORY', unitId: mixedTerminalUnit.unitId, lid: 'L-BL-RETURNED-B', label: mixedTerminalUnit.name, amount: 10 },
      ], mock: true };
      const returnedHistoryProbe = { rentalId: 'R-BL-RETURNED-HISTORY', customerId: storedBlacklisted.customerId, invoiceId: returnedHistoryInvoice.invoiceId, rentalName: 'Returned history preservation probe', startDate: '2099-10-10', endDate: '2099-10-12', status: 'Returned', units: [{ unitId: freeUnit.unitId, status: 'Returned' }, { unitId: mixedTerminalUnit.unitId, status: 'Returned' }], actions: [] };
      T.DATA.invoices.push(returnedHistoryInvoice); T.IDX.invoice.set(returnedHistoryInvoice.invoiceId, returnedHistoryInvoice);
      T.DATA.rentals.push(returnedHistoryProbe); T.IDX.rental.set(returnedHistoryProbe.rentalId, returnedHistoryProbe);
      const returnedLines = JSON.parse(JSON.stringify(returnedHistoryInvoice.lineItems));
      window.JT.setRentalStatus(returnedHistoryProbe.rentalId, 'Cancelled');
      ok(returnedHistoryProbe.status === 'Returned' && returnedHistoryProbe.units.every((eu) => eu.status === 'Returned') && returnedHistoryInvoice.lineItems.length === returnedLines.length, 'status history: a blacklisted Returned rental cannot be rewritten as Cancelled or lose its billed lines');
      returnedHistoryProbe.status = 'Returned'; returnedHistoryProbe.units.forEach((eu) => { eu.status = 'Returned'; }); returnedHistoryInvoice.lineItems = JSON.parse(JSON.stringify(returnedLines));
      window.JT.setUnitStatus(returnedHistoryProbe.rentalId, freeUnit.unitId, 'No Show');
      ok(returnedHistoryProbe.units.find((eu) => eu.unitId === freeUnit.unitId)?.status === 'Returned' && returnedHistoryInvoice.lineItems.length === returnedLines.length, 'status history: a blacklisted Returned unit cannot be rewritten as No Show or lose its billed line');
      const returnedHistoryProbeIdx = T.DATA.rentals.indexOf(returnedHistoryProbe); if (returnedHistoryProbeIdx >= 0) T.DATA.rentals.splice(returnedHistoryProbeIdx, 1); T.IDX.rental.delete(returnedHistoryProbe.rentalId);
      const returnedHistoryInvoiceIdx = T.DATA.invoices.indexOf(returnedHistoryInvoice); if (returnedHistoryInvoiceIdx >= 0) T.DATA.invoices.splice(returnedHistoryInvoiceIdx, 1); T.IDX.invoice.delete(returnedHistoryInvoice.invoiceId);
      window.JT.setDraftDate(statusProbe.rentalId, 'start', '2099-10-14');
      ok(statusProbe.startDate === '2099-10-15', 'allocation gate: an already-out blacklisted rental cannot move or clear its occupied start date');
      window.JT.setDraftDate(statusProbe.rentalId, 'end', '2099-10-16');
      ok(statusProbe.endDate === '2099-10-17', 'allocation gate: an already-out blacklisted rental cannot shorten its occupied end date');
      window.JT.setDraftDate(statusProbe.rentalId, 'end', '');
      ok(statusProbe.endDate === '2099-10-17', 'allocation gate: an already-out blacklisted rental cannot clear its occupied end date');
      window.JT.setDraftDate(statusProbe.rentalId, 'end', '2099-10-18');
      ok(statusProbe.startDate === '2099-10-15' && statusProbe.endDate === '2099-10-18', 'allocation gate: an already-out blacklisted rental may safely extend its end date for recovery');
      const mixedReturnedWindow = { rentalId: 'R-BL-OUT-RETURNED-WINDOW', customerId: storedBlacklisted.customerId, rentalName: 'Out plus returned recovery window', startDate: '2099-10-15', endDate: '2099-10-17', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }, { unitId: mixedTerminalUnit.unitId, status: 'Returned' }], actions: [] };
      T.DATA.rentals.push(mixedReturnedWindow); T.IDX.rental.set(mixedReturnedWindow.rentalId, mixedReturnedWindow);
      window.JT.setDraftDate(mixedReturnedWindow.rentalId, 'end', '2099-10-18');
      ok(mixedReturnedWindow.startDate === '2099-10-15' && mixedReturnedWindow.endDate === '2099-10-18', 'allocation recovery: a Returned sibling does not strand an On Rent unit that needs a safe end-date extension');
      const mixedCancelledWindow = { rentalId: 'R-BL-OUT-CANCELLED-WINDOW', customerId: storedBlacklisted.customerId, rentalName: 'Out plus stored Cancelled recovery window', startDate: '2099-10-15', endDate: '2099-10-17', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }, { unitId: mixedTerminalUnit.unitId, status: 'Cancelled' }], actions: [] };
      T.DATA.rentals.push(mixedCancelledWindow); T.IDX.rental.set(mixedCancelledWindow.rentalId, mixedCancelledWindow);
      window.JT.setDraftDate(mixedCancelledWindow.rentalId, 'end', '2099-10-18');
      ok(mixedCancelledWindow.startDate === '2099-10-15' && mixedCancelledWindow.endDate === '2099-10-18', 'allocation recovery: a stored Cancelled sibling does not strand an On Rent unit that needs a safe end-date extension');
      const uiSession = T.activeSession(), uiRentalCard = uiSession.cards.rentals;
      const savedRentalCard = { ...uiRentalCard }, savedMiddleCard = uiSession.cols.middle, savedWinEdit = T.__state.winEdit;
      Object.assign(uiRentalCard, { mode: 'standard', recId: mixedReturnedWindow.rentalId, recType: 'rentals' }); uiSession.cols.middle = 'rentals';
      T.__state.winEdit = null; T.render();
      const mixedRecoveryCalendar = document.querySelector(`.rdcal-edit[data-rec="${mixedReturnedWindow.rentalId}"]`);
      const mixedRecoverySplit = document.querySelector(`.js-split-open[data-rec="${mixedReturnedWindow.rentalId}"]`);
      const mixedRecoveryLocked = /Window locked/.test(mixedRecoveryCalendar?.textContent || '');
      ok(!!mixedRecoveryCalendar && !mixedRecoveryLocked && !mixedRecoverySplit, `allocation UI parity: mixed-unit recovery keeps the safe-extension calendar open but hides the hard-blocked dates split (calendar=${!!mixedRecoveryCalendar}, locked=${mixedRecoveryLocked}, split=${!!mixedRecoverySplit})`);
      Object.assign(uiRentalCard, savedRentalCard); uiSession.cols.middle = savedMiddleCard; T.__state.winEdit = savedWinEdit; T.render();
      [mixedReturnedWindow, mixedCancelledWindow].forEach((probe) => { const i = T.DATA.rentals.indexOf(probe); if (i >= 0) T.DATA.rentals.splice(i, 1); T.IDX.rental.delete(probe.rentalId); });
      statusProbe.endDate = '2099-10-17';
      T.__state.winEdit = { rentalId: statusProbe.rentalId, monthISO: '2099-10-01', anchor: null, staged: { rentalId: statusProbe.rentalId, startDate: '', endDate: '', startTime: '' } };
      T.winPickSave();
      ok(statusProbe.startDate === '2099-10-15' && statusProbe.endDate === '2099-10-17', 'allocation gate: a staged calendar Clear cannot erase an already-out blacklisted window on Save');
      const preCollisionOverbook = T.__state.overbookOn; T.__state.overbookOn = false;
      const collisionPlan = wr({ customerId: 'C0009', unitIds: [freeUnit.unitId], startDate: '2099-10-15', endDate: '2099-10-17' });
      ok(statusProbe.startDate === '2099-10-15' && statusProbe.endDate === '2099-10-17' && collisionPlan.ops.length === 0, 'allocation gate: blocked window clearing keeps the physically-out unit conflicting with a second booking');
      T.__state.overbookOn = preCollisionOverbook;
      const removalProbe = { rentalId: 'R-BL-OUT-REMOVE', customerId: storedBlacklisted.customerId, rentalName: 'Already-out removal probe', startDate: '2099-10-20', endDate: '2099-10-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent', startCapture: null, endCapture: null, fcCapture: null }], actions: [] };
      T.DATA.rentals.push(removalProbe); T.IDX.rental.set(removalProbe.rentalId, removalProbe);
      window.JT.removeUnitFromRental(removalProbe, freeUnit.unitId);
      ok(T.rentalUnits(removalProbe).some((eu) => eu.unitId === freeUnit.unitId), 'recovery gate: a blacklisted On Rent unit cannot be removed before return, even without a capture');
      const legacyRemovalProbe = { rentalId: 'R-BL-OUT-REMOVE-LEGACY', customerId: storedBlacklisted.customerId, unitId: freeUnit.unitId, units: [], rentalName: 'Legacy already-out removal probe', startDate: '2099-10-20', endDate: '2099-10-22', status: 'On Rent', actions: [] };
      T.DATA.rentals.push(legacyRemovalProbe); T.IDX.rental.set(legacyRemovalProbe.rentalId, legacyRemovalProbe);
      const legacyRemovalBlocked = window.JT.removeUnitFromRental(legacyRemovalProbe, freeUnit.unitId);
      ok(legacyRemovalBlocked === false && legacyRemovalProbe.unitId === freeUnit.unitId && T.rentalUnits(legacyRemovalProbe).some((eu) => eu.unitId === freeUnit.unitId), 'recovery gate: a legacy units:[] + unitId blacklisted On Rent unit cannot be removed before return');
      legacyRemovalProbe.status = 'Returned';
      const legacyRemovalAfterReturn = window.JT.removeUnitFromRental(legacyRemovalProbe, freeUnit.unitId);
      ok(legacyRemovalAfterReturn === true && !legacyRemovalProbe.unitId && T.rentalUnits(legacyRemovalProbe).length === 0, 'recovery gate: the same legacy blacklisted unit may be removed after it is Returned');
      const legacyRemovalProbeIdx = T.DATA.rentals.indexOf(legacyRemovalProbe); if (legacyRemovalProbeIdx >= 0) T.DATA.rentals.splice(legacyRemovalProbeIdx, 1); T.IDX.rental.delete(legacyRemovalProbe.rentalId);
      if (!T.rentalUnits(removalProbe).some((eu) => eu.unitId === freeUnit.unitId)) removalProbe.units = [{ unitId: freeUnit.unitId, status: 'Reserved', startCapture: null, endCapture: null, fcCapture: null }];
      removalProbe.status = 'Reserved'; removalProbe.units[0].status = 'Reserved';
      window.JT.removeUnitFromRental(removalProbe, freeUnit.unitId);
      ok(!T.rentalUnits(removalProbe).some((eu) => eu.unitId === freeUnit.unitId), 'recovery gate: a blacklisted Reserved unit may still be removed to release the prospective allocation');
      removalProbe.status = 'Returned'; removalProbe.units = [{ unitId: freeUnit.unitId, status: 'Returned', startCapture: null, endCapture: null, fcCapture: null }];
      window.JT.removeUnitFromRental(removalProbe, freeUnit.unitId);
      ok(!T.rentalUnits(removalProbe).some((eu) => eu.unitId === freeUnit.unitId), 'recovery gate: a blacklisted Returned unit may be removed after recovery is complete');
      const removalProbeIdx = T.DATA.rentals.indexOf(removalProbe); if (removalProbeIdx >= 0) T.DATA.rentals.splice(removalProbeIdx, 1); T.IDX.rental.delete(removalProbe.rentalId);
      const captureInvoice = { invoiceId: 'I-BL-CAPTURE-BACKFILL', customerId: storedBlacklisted.customerId, rentalIds: ['R-BL-CAPTURE-BACKFILL'], amountPaid: 0, lineItems: [{ kind: 'rental', ref: 'R-BL-CAPTURE-BACKFILL', unitId: freeUnit.unitId, lid: 'L-BL-CAPTURE', label: freeUnit.name, amount: 10 }], mock: true };
      const captureProbe = { rentalId: 'R-BL-CAPTURE-BACKFILL', customerId: storedBlacklisted.customerId, invoiceId: captureInvoice.invoiceId, rentalName: 'Already-out capture backfill probe', startDate: '2099-10-20', endDate: '2099-10-22', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent', startCapture: null, endCapture: null, fcCapture: null }], actions: [] };
      T.DATA.invoices.push(captureInvoice); T.IDX.invoice.set(captureInvoice.invoiceId, captureInvoice);
      T.DATA.rentals.push(captureProbe); T.IDX.rental.set(captureProbe.rentalId, captureProbe);
      const captureInputsBefore = document.querySelectorAll('input[type="file"][capture="environment"]').length;
      T.yardCapture(captureProbe.rentalId, 'start', freeUnit.unitId);
      const startCaptureInputs = [...document.querySelectorAll('input[type="file"][capture="environment"]')];
      ok(startCaptureInputs.length === captureInputsBefore + 1, 'recovery gate: a blacklisted unit already stored On Rent may open Start/Delivery to backfill missing evidence');
      startCaptureInputs.slice(captureInputsBefore).forEach((input) => input.remove());
      T.commitYardCapture(captureProbe.rentalId, 'start', freeUnit.unitId, 'data:video/mp4;base64,AA==');
      ok(captureProbe.units[0].status === 'On Rent' && !!captureProbe.units[0].startCapture, 'recovery gate: Start/Delivery backfill records evidence without reacquiring or rewinding the already-out unit');
      const endInputsBefore = document.querySelectorAll('input[type="file"][capture="environment"]').length;
      T.yardCapture(captureProbe.rentalId, 'end', freeUnit.unitId);
      const endCaptureInputs = [...document.querySelectorAll('input[type="file"][capture="environment"]')];
      ok(endCaptureInputs.length === endInputsBefore + 1, 'recovery gate: a backfilled delivery unlocks the normal End/Recovery capture path');
      endCaptureInputs.slice(endInputsBefore).forEach((input) => input.remove());
      T.commitYardCapture(captureProbe.rentalId, 'end', freeUnit.unitId, 'data:video/mp4;base64,AA==');
      ok(captureProbe.units[0].status === 'Returned' && !!captureProbe.units[0].endCapture, 'recovery gate: the blacklisted unit can complete evidence-backed return after the backfill');
      const captureProbeIdx = T.DATA.rentals.indexOf(captureProbe); if (captureProbeIdx >= 0) T.DATA.rentals.splice(captureProbeIdx, 1); T.IDX.rental.delete(captureProbe.rentalId);
      const captureInvoiceIdx = T.DATA.invoices.indexOf(captureInvoice); if (captureInvoiceIdx >= 0) T.DATA.invoices.splice(captureInvoiceIdx, 1); T.IDX.invoice.delete(captureInvoice.invoiceId);
      const statusProbeIdx = T.DATA.rentals.indexOf(statusProbe); if (statusProbeIdx >= 0) T.DATA.rentals.splice(statusProbeIdx, 1); T.IDX.rental.delete(statusProbe.rentalId);
      const otherWindowUnit = T.DATA.units.find((u) => u.unitId !== freeUnit.unitId);
      const windowProbe = { rentalId: 'R-BL-WINDOW', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Blacklist window probe', startDate: '2099-11-10', endDate: '2099-11-12', startTime: '', status: 'Reserved', units: [{ unitId: freeUnit.unitId, status: 'Reserved' }, { unitId: otherWindowUnit.unitId, status: 'Reserved' }], actions: [] };
      T.DATA.rentals.push(windowProbe); T.IDX.rental.set(windowProbe.rentalId, windowProbe);
      window.JT.setRentalStatus(windowProbe.rentalId, 'Cancelled');
      ok(windowProbe.status === 'Cancelled' && windowProbe.units.every((eu) => eu.status === 'Cancelled'), 'status gate: a prospective blacklisted rental may still be Cancelled to release its booking');
      windowProbe.status = 'Reserved'; windowProbe.units.forEach((eu) => { eu.status = 'Reserved'; });
      window.JT.setUnitStatus(windowProbe.rentalId, freeUnit.unitId, 'No Show');
      ok(windowProbe.units.find((eu) => eu.unitId === freeUnit.unitId)?.status === 'No Show', 'status gate: a prospective blacklisted unit may still be marked No Show to release its booking');
      windowProbe.status = 'Reserved'; windowProbe.units.forEach((eu) => { eu.status = 'Reserved'; });
      window.JT.setRentalStatus(windowProbe.rentalId, 'Quote');
      ok(windowProbe.status === 'Reserved' && windowProbe.units.every((eu) => eu.status === 'Reserved'), 'status gate: a Reserved blacklisted rental cannot move backward to Quote and hide its booking');
      windowProbe.status = 'Reserved'; windowProbe.units.forEach((eu) => { eu.status = 'Reserved'; });
      window.JT.setUnitStatus(windowProbe.rentalId, freeUnit.unitId, 'Quote');
      ok(windowProbe.status === 'Reserved' && windowProbe.units.every((eu) => eu.status === 'Reserved'), 'status gate: a Reserved blacklisted unit cannot move backward to Quote and appear available');
      windowProbe.status = 'Reserved'; windowProbe.units.forEach((eu) => { eu.status = 'Reserved'; });
      window.JT.setDraftDate(windowProbe.rentalId, 'start', '2099-12-01');
      ok(windowProbe.startDate === '2099-11-10', 'allocation gate: direct date edits cannot rebook a prospective blacklisted rental');
      windowProbe.startDate = '2099-11-10'; windowProbe.endDate = '2099-11-12';
      T.__state.winEdit = { rentalId: windowProbe.rentalId, monthISO: '2099-11-01', anchor: null };
      T.winPickDay('2099-11-20');
      ok(windowProbe.startDate === '2099-11-10' && windowProbe.endDate === '2099-11-12', 'allocation gate: inline calendar taps cannot rebook a prospective blacklisted rental');
      windowProbe.startDate = '2099-11-10'; windowProbe.endDate = '2099-11-12';
      T.__state.winEdit = { rentalId: windowProbe.rentalId, monthISO: '2099-11-01', anchor: null, staged: { rentalId: windowProbe.rentalId, startDate: '2099-12-05', endDate: '2099-12-08', startTime: '' } };
      T.winPickSave();
      ok(windowProbe.startDate === '2099-11-10' && windowProbe.endDate === '2099-11-12', 'allocation gate: staged calendar save cannot rebook a prospective blacklisted rental');
      const windowUnitsBefore = JSON.parse(JSON.stringify(windowProbe.units)), rentalsBeforeSplit = T.DATA.rentals.length;
      const blockedSplit = window.JT.splitUnitToNewRental(windowProbe.rentalId, otherWindowUnit.unitId, '2099-12-10', '2099-12-12');
      ok(blockedSplit === null && T.DATA.rentals.length === rentalsBeforeSplit && JSON.stringify(windowProbe.units) === JSON.stringify(windowUnitsBefore), 'allocation gate: splitting dates cannot create a sibling booking for a prospective blacklisted unit');
      if (blockedSplit) { const splitIdx = T.DATA.rentals.indexOf(blockedSplit); if (splitIdx >= 0) T.DATA.rentals.splice(splitIdx, 1); T.IDX.rental.delete(blockedSplit.rentalId); windowProbe.units = windowUnitsBefore; }
      const windowProbeIdx = T.DATA.rentals.indexOf(windowProbe); if (windowProbeIdx >= 0) T.DATA.rentals.splice(windowProbeIdx, 1); T.IDX.rental.delete(windowProbe.rentalId);
      const noShowProbe = { rentalId: 'R-BL-NO-SHOW-WINDOW', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Derived No Show window probe', startDate: '2000-01-10', endDate: '2000-01-12', startTime: '', status: 'Reserved', units: [{ unitId: freeUnit.unitId, status: 'Reserved' }], actions: [] };
      T.DATA.rentals.push(noShowProbe); T.IDX.rental.set(noShowProbe.rentalId, noShowProbe);
      window.JT.setDraftDate(noShowProbe.rentalId, 'end', '2099-12-22');
      window.JT.setDraftDate(noShowProbe.rentalId, 'start', '2099-12-20');
      ok(noShowProbe.startDate === '2000-01-10' && noShowProbe.endDate === '2000-01-12', 'allocation gate: a derived No Show cannot be re-dated in two steps into a future blacklisted reservation');
      const noShowProbeIdx = T.DATA.rentals.indexOf(noShowProbe); if (noShowProbeIdx >= 0) T.DATA.rentals.splice(noShowProbeIdx, 1); T.IDX.rental.delete(noShowProbe.rentalId);
      const mixedNoShowProbe = { rentalId: 'R-BL-MIXED-NO-SHOW-WINDOW', customerId: storedBlacklisted.customerId, invoiceId: null, rentalName: 'Already-out plus derived No Show window probe', startDate: '2000-01-10', endDate: '2000-01-12', startTime: '', status: 'On Rent', units: [{ unitId: freeUnit.unitId, status: 'On Rent' }, { unitId: otherWindowUnit.unitId, status: 'Reserved' }], actions: [] };
      T.DATA.rentals.push(mixedNoShowProbe); T.IDX.rental.set(mixedNoShowProbe.rentalId, mixedNoShowProbe);
      window.JT.setDraftDate(mixedNoShowProbe.rentalId, 'end', '2099-12-22');
      window.JT.setDraftDate(mixedNoShowProbe.rentalId, 'start', '2099-12-20');
      ok(mixedNoShowProbe.startDate === '2000-01-10' && mixedNoShowProbe.endDate === '2000-01-12', 'allocation gate: an On Rent sibling cannot hide a derived No Show while re-dating a blacklisted rental');
      const mixedNoShowProbeIdx = T.DATA.rentals.indexOf(mixedNoShowProbe); if (mixedNoShowProbeIdx >= 0) T.DATA.rentals.splice(mixedNoShowProbeIdx, 1); T.IDX.rental.delete(mixedNoShowProbe.rentalId);
      const legacyQuoteProbe = { rentalId: 'R-BL-QUOTE-LEGACY', customerId: 'C-BL', rentalName: 'Legacy blacklist quote probe', startDate: '', endDate: '', startTime: '', status: 'Quote', units: [], actions: [] };
      T.DATA.rentals.push(legacyQuoteProbe); T.IDX.rental.set(legacyQuoteProbe.rentalId, legacyQuoteProbe);
      window.JT.setDraftDate(legacyQuoteProbe.rentalId, 'start', '2099-10-04'); window.JT.setDraftDate(legacyQuoteProbe.rentalId, 'end', '2099-10-06');
      ok(legacyQuoteProbe.status === 'Quote', 'human rent: dating a Quote for a legacy accountType blacklist cannot turn it into Reserved');
      legacyQuoteProbe.units = [{ unitId: freeUnit.unitId, status: 'Quote' }];
      window.JT.setRentalStatus(legacyQuoteProbe.rentalId, 'Off Rent');
      ok(legacyQuoteProbe.status === 'Quote' && legacyQuoteProbe.units[0].status === 'Quote', 'status gate: a legacy accountType blacklist cannot jump a Quote master directly to Off Rent');
      window.JT.setUnitStatus(legacyQuoteProbe.rentalId, freeUnit.unitId, 'End Rent');
      ok(legacyQuoteProbe.status === 'Quote' && legacyQuoteProbe.units[0].status === 'Quote', 'status gate: a legacy accountType blacklist cannot jump a Quote unit directly to End Rent');
      const legacyQuoteProbeIdx = T.DATA.rentals.indexOf(legacyQuoteProbe); if (legacyQuoteProbeIdx >= 0) T.DATA.rentals.splice(legacyQuoteProbeIdx, 1); T.IDX.rental.delete(legacyQuoteProbe.rentalId);
      const storedBlacklistedIdx = T.DATA.customers.indexOf(storedBlacklisted); if (storedBlacklistedIdx >= 0) T.DATA.customers.splice(storedBlacklistedIdx, 1); T.IDX.customer.delete(storedBlacklisted.customerId);
    ...
    // 22a) D12/D17 — manual Blacklist is independent of the automatic card/payment gate.
    // Every non-blacklisted account keeps the Block Account action; only a stored blacklist swaps
    // it for Lift Blacklist. Automatic states retain their badge + self-clear explanation.
    {
      const activeCard = { id: 'K-BLOCK', status: 'active', isDefault: true, expMonth: 12, expYear: 2099 };
      const heldInv = { invoiceId: 'I-BLOCK-HOLD', amountPaid: 0, lineItems: [{ kind: 'custom', lid: 'L-BLOCK', label: 'Open balance', amount: 10 }] };
      const legacyCustomer = { customerId: 'C-BLOCK-LEGACY', cards: [activeCard], accountType: 'Blacklisted', _prevAccountType: 'Non-Business' };
      const collectionsCustomer = { customerId: 'C-BLOCK-COL', cards: [activeCard], accountType: 'Blacklisted' };
      const collectionsInv = { invoiceId: 'I-BLOCK-COL', customerId: collectionsCustomer.customerId, amountPaid: 0, collections: { status: 'Queued' }, lineItems: [{ kind: 'custom', lid: 'L-BLOCK-COL', label: 'Collections balance', amount: 10 }] };
      T.DATA.invoices.push(heldInv, collectionsInv); T.IDX.invoice.set(heldInv.invoiceId, heldInv); T.IDX.invoice.set(collectionsInv.invoiceId, collectionsInv);
      const cases = [
        ['clear account', { customerId: 'C-BLOCK-CLEAR', cards: [activeCard] }, true, false, false, ''],
        ['no-card automatic block', { customerId: 'C-BLOCK-NOCARD', cards: [] }, true, false, true, 'No card on file'],
        ['failed-payment automatic block', { customerId: 'C-BLOCK-FAIL', cards: [activeCard], chargeFailedAt: T.TODAY_ISO }, true, false, true, 'Payment failed'],
        ['invoice hold', { customerId: 'C-BLOCK-HOLD', cards: [activeCard], block: { type: 'invoice-hold', invoiceIds: [heldInv.invoiceId] } }, true, false, false, 'Held — invoice(s) unpaid'],
        ['stored blacklist', { customerId: 'C-BLOCK-BL', cards: [], block: { type: 'blacklist' } }, false, true, false, 'Blacklisted'],
        ['legacy blacklist', legacyCustomer, false, true, false, 'Blacklisted'],
        ['active collections blacklist', collectionsCustomer, false, false, false, 'Blacklisted'],
      ];
      cases.forEach(([label, customer, wantsBlock, wantsLift, wantsAutoCopy, badgeLabel]) => {
        const html = T.acctBlockFoot(customer);
        ok(/js-block-account/.test(html) === wantsBlock, `account block: ${label} ${wantsBlock ? 'offers Block Account' : 'does not offer Block Account'}`);
        ok(/js-lift-blacklist/.test(html) === wantsLift, `account block: ${label} ${wantsLift ? 'offers Lift Blacklist' : 'does not offer Lift Blacklist'}`);
        ok(/clears automatically/.test(html) === wantsAutoCopy, `account block: ${label} ${wantsAutoCopy ? 'keeps automatic-clear copy' : 'has no automatic-clear copy'}`);
        if (badgeLabel) ok(html.includes(badgeLabel), `account block: ${label} keeps its "${badgeLabel}" badge`);
      });
      ok(/recall collections to lift/i.test(T.acctBlockFoot(collectionsCustomer)), 'account block: an active Collections blacklist explains that Recall owns the lift');
      const collectionsLift = T.liftCustomerBlacklist(collectionsCustomer, 'Test Admin');
      ok(collectionsLift.ok === false && collectionsLift.reason === 'collections' && collectionsCustomer.accountType === 'Blacklisted', 'account block: active Collections owns its blacklist; the manual lift cannot falsely clear it');
      collectionsCustomer.accountType = 'Business';   // simulate Agreement/Card or Membership rewriting the legacy field
      const overwrittenCollectionsHtml = T.acctBlockFoot(collectionsCustomer);
      ok(/Blacklisted/.test(overwrittenCollectionsHtml) && !/js-block-account/.test(overwrittenCollectionsHtml) && !/js-lift-blacklist/.test(overwrittenCollectionsHtml), 'account block: active Collections remains visibly blacklisted after another account-type writer runs');
      const overwrittenCollectionsLift = T.liftCustomerBlacklist(collectionsCustomer, 'Test Admin');
      ok(overwrittenCollectionsLift.ok === false && overwrittenCollectionsLift.reason === 'collections' && collectionsCustomer.accountType === 'Business', 'account block: account-type writers cannot create a false Collections lift');
      const collectionsMoveTarget = { customerId: 'C-BLOCK-COL-TARGET', name: 'Collections move target', cards: [activeCard], accountType: 'Business' };
      T.DATA.customers.push(collectionsMoveTarget); T.IDX.customer.set(collectionsMoveTarget.customerId, collectionsMoveTarget);
      ok(T.DROP_MATRIX.customers.invoices(collectionsMoveTarget, collectionsInv) === false && T.DROP_MATRIX.invoices.customers(collectionsInv, collectionsMoveTarget) === false, 'collections freeze: an active placement never lights as a customer-reassignment drop');
      const collectionsMove = window.JT.setInvoiceCustomer(collectionsInv.invoiceId, collectionsMoveTarget.customerId);
      ok(collectionsMove === null && collectionsInv.customerId === collectionsCustomer.customerId, 'collections freeze: the hard customer-reassignment mutation refuses an active placement');
      const collectionsRemove = window.JT.clearInvoiceCustomer(collectionsInv);
      ok(collectionsRemove === false && collectionsInv.customerId === collectionsCustomer.customerId, 'collections freeze: the hard customer-removal mutation refuses an active placement');
      ok(/Blacklisted/.test(T.acctBlockFoot(collectionsCustomer)) && !/js-block-account/.test(T.acctBlockFoot(collectionsCustomer)), 'collections freeze: a refused reassignment cannot falsely lift the original account after an account-type rewrite');
      const emptyCollectionsDraft = { invoiceId: 'I-BLOCK-COL-EMPTY', customerId: collectionsCustomer.customerId, amountPaid: 0, collections: { status: 'Queued' }, lineItems: [], rentalIds: [], mock: true };
      ok(T.isEmptyMockDraft('invoices', emptyCollectionsDraft) === false, 'collections freeze: an active placement is never sweepable even if every editable structural field is empty');
      const collectionsLinkedRental = { rentalId: 'R-BLOCK-COL-LINKED', customerId: collectionsCustomer.customerId, invoiceId: collectionsInv.invoiceId, units: [], actions: [] };
      collectionsInv.rentalIds = [collectionsLinkedRental.rentalId];
      T.DATA.rentals.push(collectionsLinkedRental); T.IDX.rental.set(collectionsLinkedRental.rentalId, collectionsLinkedRental);
      const wrCollectionsUnlinkPlan = T.WR_OPERATIONS.unlinkInvoice.validate({ rentalId: collectionsLinkedRental.rentalId });
      ok(/Collections/i.test(wrCollectionsUnlinkPlan.issue || ''), 'collections freeze: Mr. Wrangler refuses to plan an invoice unlink before Recall');
      const wrCollectionsUnlink = T.WR_OPERATIONS.unlinkInvoice.apply({ rentalId: collectionsLinkedRental.rentalId });
      ok(wrCollectionsUnlink === null && collectionsLinkedRental.invoiceId === collectionsInv.invoiceId && collectionsInv.rentalIds.includes(collectionsLinkedRental.rentalId), 'collections freeze: Mr. Wrangler hard apply cannot unlink an active placement');
      const collectionsLinkedIdx = T.DATA.rentals.indexOf(collectionsLinkedRental); if (collectionsLinkedIdx >= 0) T.DATA.rentals.splice(collectionsLinkedIdx, 1); T.IDX.rental.delete(collectionsLinkedRental.rentalId); collectionsInv.rentalIds = [];

      const manualInvoiceOwner = { customerId: 'C-BLOCK-INVOICE-OWNER', name: 'Manual blacklist invoice owner', cards: [activeCard], block: { type: 'blacklist' }, actions: [] };
      const manualOwnerInv = { invoiceId: 'I-BLOCK-INVOICE-OWNER', customerId: manualInvoiceOwner.customerId, amountPaid: 0, lineItems: [{ kind: 'rental', lid: 'L-BLOCK-INVOICE-OWNER', ref: 'R-BLOCK-INVOICE-OWNER', unitId: T.DATA.units[0].unitId, label: 'Legacy ownership', amount: 10 }], rentalIds: ['R-BLOCK-INVOICE-OWNER'] };
      const manualOwnerRental = { rentalId: 'R-BLOCK-INVOICE-OWNER', customerId: null, invoiceId: manualOwnerInv.invoiceId, rentalName: 'Legacy customerless rental', startDate: '2099-09-10', endDate: '2099-09-12', status: 'Quote', units: [{ unitId: T.DATA.units[0].unitId, status: 'Quote' }], actions: [] };
      T.DATA.customers.push(manualInvoiceOwner); T.IDX.customer.set(manualInvoiceOwner.customerId, manualInvoiceOwner);
      T.DATA.invoices.push(manualOwnerInv); T.IDX.invoice.set(manualOwnerInv.invoiceId, manualOwnerInv);
      T.DATA.rentals.push(manualOwnerRental); T.IDX.rental.set(manualOwnerRental.rentalId, manualOwnerRental);
      ok(T.invoiceRentalLinkFrozen(manualOwnerInv, manualOwnerInv.lineItems[0]) === true, 'blacklisted invoice owner: its rental line is a frozen ownership edge, not a removable bypass');
      const wrManualUnlinkPlan = T.WR_OPERATIONS.unlinkInvoice.validate({ rentalId: manualOwnerRental.rentalId });
      ok(/blacklist|lift|recall/i.test(wrManualUnlinkPlan.issue || ''), 'blacklisted invoice owner: Mr. Wrangler refuses to plan removal of the rental\'s last blacklisted owner edge');
      const wrManualUnlink = T.WR_OPERATIONS.unlinkInvoice.apply({ rentalId: manualOwnerRental.rentalId });
      ok(wrManualUnlink === null && manualOwnerRental.invoiceId === manualOwnerInv.invoiceId && manualOwnerInv.rentalIds.includes(manualOwnerRental.rentalId), 'blacklisted invoice owner: Mr. Wrangler hard apply cannot erase the last owner edge');
      // Failed-before isolation: the old hard apply detached both sides even after validate should refuse.
      manualOwnerRental.invoiceId = manualOwnerInv.invoiceId; manualOwnerInv.rentalIds = [manualOwnerRental.rentalId];
      ok(T.invoiceVoidable(manualOwnerInv) === false, 'blacklisted invoice owner: an otherwise-unpaid invoice is not voidable when voiding would erase the rental\'s last owner edge');
      T.voidInvoice(manualOwnerInv);
      ok(!manualOwnerInv.voided && manualOwnerRental.invoiceId === manualOwnerInv.invoiceId && manualOwnerInv.rentalIds.includes(manualOwnerRental.rentalId), 'blacklisted invoice owner: the hard void path cannot erase the last owner edge');
      // Failed-before isolation: restore the fixture if the old void path mutated it.
      manualOwnerRental.invoiceId = manualOwnerInv.invoiceId; manualOwnerInv.rentalIds = [manualOwnerRental.rentalId]; delete manualOwnerInv.voided; delete manualOwnerInv.voidedAt; delete manualOwnerInv.voidedBy;
      manualOwnerRental.status = 'On Rent'; manualOwnerRental.units[0].status = 'On Rent';
      ok(T.invoiceRentalLinkFrozen(manualOwnerInv, manualOwnerInv.lineItems[0]) === true, 'blacklisted invoice owner: already-out recovery stays legal, but its sole owner edge remains frozen against detach');
      manualOwnerRental.status = 'Quote'; manualOwnerRental.units[0].status = 'Quote';
      const directOwnedRental = { rentalId: 'R-BLOCK-DIRECT-OWNER', customerId: manualInvoiceOwner.customerId, invoiceId: manualOwnerInv.invoiceId, status: 'Quote', units: [], actions: [] };
      const directOwnedLine = { kind: 'rental', lid: 'L-BLOCK-DIRECT-OWNER', ref: directOwnedRental.rentalId, label: 'Direct ownership survives', amount: 10 };
      T.DATA.rentals.push(directOwnedRental); T.IDX.rental.set(directOwnedRental.rentalId, directOwnedRental);
      ok(T.invoiceRentalLinkFrozen(manualOwnerInv, directOwnedLine) === false, 'blacklisted direct owner: an invoice line is removable when the rental\'s direct blacklisted customer edge survives');
      T.DATA.rentals = T.DATA.rentals.filter((r) => r !== directOwnedRental); T.IDX.rental.delete(directOwnedRental.rentalId);
      ok(T.DROP_MATRIX.customers.invoices(collectionsMoveTarget, manualOwnerInv) === false && T.DROP_MATRIX.invoices.customers(manualOwnerInv, collectionsMoveTarget) === false, 'blacklisted invoice owner: reassignment never lights as a valid customer drop');
      const clearManualOwner = window.JT.clearInvoiceCustomer(manualOwnerInv);
      ok(clearManualOwner === false && manualOwnerInv.customerId === manualInvoiceOwner.customerId, 'blacklisted invoice owner: hard customer removal cannot erase the rental blacklist fallback');
      const moveManualOwner = window.JT.setInvoiceCustomer(manualOwnerInv.invoiceId, collectionsMoveTarget.customerId);
      ok(moveManualOwner === null && manualOwnerInv.customerId === manualInvoiceOwner.customerId, 'blacklisted invoice owner: hard reassignment cannot erase the rental blacklist fallback');
      const mergeSource = { invoiceId: 'I-BLOCK-MERGE-SRC', customerId: manualInvoiceOwner.customerId, amountPaid: 0, lineItems: [{ kind: 'rental', lid: 'L-BLOCK-MERGE-SRC', ref: 'R-BLOCK-MERGE', amount: 10 }], rentalIds: [] };
      const mergeKeep = { invoiceId: 'I-BLOCK-MERGE-KEEP', customerId: manualInvoiceOwner.customerId, amountPaid: 0, lineItems: [], rentalIds: [] };
      const mergeRental = { rentalId: 'R-BLOCK-MERGE', customerId: null, invoiceId: mergeSource.invoiceId, status: 'Quote', units: [], actions: [] };
      T.DATA.invoices.push(mergeSource, mergeKeep); T.IDX.invoice.set(mergeSource.invoiceId, mergeSource); T.IDX.invoice.set(mergeKeep.invoiceId, mergeKeep);
      T.DATA.rentals.push(mergeRental); T.IDX.rental.set(mergeRental.rentalId, mergeRental);
      T.mergeInvoiceInto(mergeKeep.invoiceId, mergeSource.invoiceId);
      ok(mergeRental.invoiceId === mergeKeep.invoiceId && mergeKeep.rentalIds.includes(mergeRental.rentalId) && T.rentalAccountCustomer(mergeRental) === manualInvoiceOwner, 'blacklisted invoice owner: merging a legacy pointer-only invoice preserves and repoints its sole owner edge');
      T.DATA.invoices = T.DATA.invoices.filter((i) => i !== mergeSource && i !== mergeKeep); T.IDX.invoice.delete(mergeSource.invoiceId); T.IDX.invoice.delete(mergeKeep.invoiceId);
      T.DATA.rentals = T.DATA.rentals.filter((r) => r !== mergeRental); T.IDX.rental.delete(mergeRental.rentalId);
      manualOwnerInv.customerId = null;   // simulate a legacy record already stripped before this fix
      window.JT.setRentalStatus(manualOwnerRental.rentalId, 'On Rent');
      ok(manualOwnerRental.status === 'Quote' && manualOwnerRental.units[0].status === 'Quote', 'customer identity gate: a legacy invoiced rental with no effective customer fails closed before On Rent');
      manualOwnerRental.status = 'On Rent'; manualOwnerRental.units[0].status = 'On Rent';
      window.JT.setRentalStatus(manualOwnerRental.rentalId, 'End Rent');
      ok(manualOwnerRental.status === 'End Rent' && manualOwnerRental.units[0].status === 'End Rent', 'customer identity gate: a legacy already-out rental can still progress through recovery even if its customer link is missing');
      const manualOwnerRentalIdx = T.DATA.rentals.indexOf(manualOwnerRental); if (manualOwnerRentalIdx >= 0) T.DATA.rentals.splice(manualOwnerRentalIdx, 1); T.IDX.rental.delete(manualOwnerRental.rentalId);
      const manualOwnerInvIdx = T.DATA.invoices.indexOf(manualOwnerInv); if (manualOwnerInvIdx >= 0) T.DATA.invoices.splice(manualOwnerInvIdx, 1); T.IDX.invoice.delete(manualOwnerInv.invoiceId);
      T.DATA.customers = T.DATA.customers.filter((c) => c !== manualInvoiceOwner); T.IDX.customer.delete(manualInvoiceOwner.customerId);
      if (collectionsInv.customerId !== collectionsCustomer.customerId) collectionsInv.customerId = collectionsCustomer.customerId;   // failed-before cleanup keeps later tests isolated
      T.DATA.customers = T.DATA.customers.filter((c) => c !== collectionsMoveTarget); T.IDX.customer.delete(collectionsMoveTarget.customerId);
      const mixedBlacklist = { customerId: 'C-BLOCK-MIXED', cards: [activeCard], accountType: 'Blacklisted', _prevAccountType: 'Business', block: { type: 'blacklist' }, actions: [] };
      T.DATA.customers.push(mixedBlacklist); T.IDX.customer.set(mixedBlacklist.customerId, mixedBlacklist);
      const mixedLift = T.liftCustomerBlacklist(mixedBlacklist, 'Test Admin');
      ok(mixedLift.ok === true && !mixedBlacklist.block && mixedBlacklist.accountType === 'Business' && !mixedBlacklist._prevAccountType, 'account block: one approved lift clears both stored and legacy blacklist representations');
      T.DATA.customers = T.DATA.customers.filter((c) => c !== mixedBlacklist); T.IDX.customer.delete(mixedBlacklist.customerId);
      const legacyWithHold = { customerId: 'C-BLOCK-LEGACY-HOLD', cards: [activeCard], accountType: 'Blacklisted', _prevAccountType: 'Business', block: { type: 'invoice-hold', invoiceIds: [heldInv.invoiceId] }, actions: [] };
      T.DATA.customers.push(legacyWithHold); T.IDX.customer.set(legacyWithHold.customerId, legacyWithHold);
      const legacyWithHoldLift = T.liftCustomerBlacklist(legacyWithHold, 'Test Admin');
      ok(legacyWithHoldLift.ok === true && legacyWithHold.accountType === 'Business' && legacyWithHold.block?.type === 'invoice-hold', 'account block: lifting a legacy blacklist preserves an independent unpaid-invoice hold');
      T.DATA.customers = T.DATA.customers.filter((c) => c !== legacyWithHold); T.IDX.customer.delete(legacyWithHold.customerId);
      const overwrittenLegacy = { customerId: 'C-BLOCK-LEGACY-OVERWRITE', cards: [activeCard], accountType: 'Business', blacklistedAt: '2026-07-09', _prevAccountType: 'Non-Business', actions: [] };
      T.DATA.customers.push(overwrittenLegacy); T.IDX.customer.set(overwrittenLegacy.customerId, overwrittenLegacy);
      ok(/Blacklisted/.test(T.acctBlockFoot(overwrittenLegacy)) && /js-lift-blacklist/.test(T.acctBlockFoot(overwrittenLegacy)), 'account block: the persisted legacy marker pair survives a later account-type overwrite');
      const overwrittenLegacyLift = T.liftCustomerBlacklist(overwrittenLegacy, 'Test Admin');
      ok(overwrittenLegacyLift.ok === true && overwrittenLegacy.accountType === 'Business' && !('blacklistedAt' in overwrittenLegacy) && !('_prevAccountType' in overwrittenLegacy), 'account block: approved lift clears the legacy marker pair without undoing a newer account type');
      T.DATA.customers = T.DATA.customers.filter((c) => c !== overwrittenLegacy); T.IDX.customer.delete(overwrittenLegacy.customerId);
      const alreadyLiftedLegacy = { customerId: 'C-BLOCK-LEGACY-LIFTED', cards: [activeCard], accountType: 'Business', blacklistedAt: '2026-07-09' };
      ok(!/Blacklisted/.test(T.acctBlockFoot(alreadyLiftedLegacy)) && /js-block-account/.test(T.acctBlockFoot(alreadyLiftedLegacy)), 'account block: blacklistedAt alone does not resurrect a legacy blacklist that was already lifted');
      const emptyPrevLegacy = { customerId: 'C-BLOCK-LEGACY-EMPTY-PREV', cards: [activeCard], accountType: 'Business', blacklistedAt: '2026-07-09', _prevAccountType: '' };
      ok(/Blacklisted/.test(T.acctBlockFoot(emptyPrevLegacy)) && /js-lift-blacklist/.test(T.acctBlockFoot(emptyPrevLegacy)), 'account block: an empty-string legacy previous type still forms an active persisted blacklist marker pair');
      T.DATA.invoices = T.DATA.invoices.filter((i) => i !== heldInv && i !== collectionsInv); T.IDX.invoice.delete(heldInv.invoiceId); T.IDX.invoice.delete(collectionsInv.invoiceId);
    }
    ...
    // logging case clears it first (errLogClear) and every captured line is kept in allLogs for the key check.
    {
      ...
      try {
        ...
        const toCodeResend = () => { toCode('personal'); T.pidUI._phone = '(337) 555-0171'; T.pidUI._sent = null; localStorage.setItem('jactec.pidRoster', ROSTER); };
        ...
        Object.assign(T.pidUI, { step: 'identify', personId: '', name: '', masked: '', kind: '', err: '', _phone: '', _tok: '', _role: '', _mintAt: 0, _sent: null });
