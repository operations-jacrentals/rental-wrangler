...
| Copy → paste | `chatDropCopy` · `state.held` · message `refs` | Drag a record onto the chat pad → paste as a **live, clickable chip** (`chatRefChipHtml`; a deleted record greys out). Mr. Wrangler paste sets the chat's *focused record* instead (`wranglerContext`). **Internal-only** — never surfaces in a customer text/email composer. (The right-click "Copy to chat" entry was retired as a duplicate of Copy, 2026-07-17.) |
