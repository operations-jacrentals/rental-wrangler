...
- **D3 — One invoice per placement; multi-invoice debt is handled by INVOICE MERGE first (OQ-9).** Keep placement to a single invoice for clean money math. To place a customer's whole stack, the office **merges the open invoices into one** *before* placing it — and **the merge MUST carry line items that reference their origin invoices** (so the merged invoice is auditable back to each source). **Action item:** confirm the invoice-merge capability is actually built (`invoicing-payments` references a same-customer merge constraint in §3.3, but it may be spec-only) — **if not built, building origin-referencing invoice merge is a prerequisite for the "place a whole customer's debt" workflow.** Cross-customer placement stays hard-rejected (`cross-customer`). This makes **`invoicing-payments` invoice-merge a dependency** of the multi-invoice collections workflow.
