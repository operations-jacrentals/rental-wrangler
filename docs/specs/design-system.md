...
A reconciliation pass against `main` after today's `staging` → `main` promotion. **Overall maturity claim (line 7, "✅ Shipped, deeply enforced") still holds** — nothing below contradicts the baseline, but the rule ceiling has moved again since the last truth-up (`fc34cb0`, 2026-07-08, which brought §2.2 to R0–R27) and one new icon-system capability shipped that this spec never described.

- **R-rulebook ceiling is now R30, not R27.** Two feature merges on 2026-07-09 claimed three more stamped rules: **R28** (`Account button` / `acctBtn`) and **R29** (`Invoice action menu` / `invoiceStatMenu`), both from the *Customer Details reorg* (retiring the Invoice card, embedding invoices — PR #566); and **R30** (`Paused banner` / `.wr-paused`), from *Wrangler Ops* (the developer live-chat bridge — PR #565). Live source: `RULE_META` (`app.js:5319–5321`). §2.2's table and its trailing "Note" (still anchored on R25→R27) are now one step further stale — SEE the updated table/note below. AC #12 and §11 Q1 (both still say "today R25") are stale in the same way — this is the exact "drift between docs and code" failure mode §10 already names, recurring within days of the last truth-up.
- **NEW capability, not covered by this spec at all: hand-drawn hover-sprite icon frames (`icons-frames.js`, `CATEGORY_FRAMES`).** Built DIFFERENTLY than anything in §2/§5 of this doc describes: 2–3 hand-drawn poses per category-glyph family (excavator, dozer, scissor, telehandler, towablelift, roller, trencher, buggy/dumptrailer as 2-frame toggles, …), drawn directly in Figma by Jac, hard-cut via a CSS `steps()` keyframe cycle (`style.css` "CATEGORY FRAME SPRITES", 2026-07-07) instead of a continuous transform loop. Shipped 2026-07-08 (PR #555), then two correction passes: PR #558 (2026-07-08, corrected the 3-frame cycle order to match Jac's actual drawn frames) and PR #561 (2026-07-09, fixed a dropped Figma vector detail). This is a **third** icon-authoring mechanism alongside the two this spec's §5.3 and CLAUDE.md already cover (Lucide-vendored statics via `tools/gen-icons.mjs`, and the Lottie-derived `CATEGORY_ANIM` continuous loops in `icons-anim.js`) — §5.3's one-liner ("icons are vendored verbatim into `icons.js`… never live-fetched") only speaks to the Lucide path and doesn't mention this hand-authored one. **Gap, not contradiction:** nothing in this spec is wrong about it, it's simply undocumented here. Note also that CLAUDE.md's own "Icons" section — despite covering the general hand-drawn/bespoke exception policy and its `CATEGORY_ANIM` sibling in detail — does not yet name `icons-frames.js` / `CATEGORY_FRAMES` specifically either; that section could use the same truth-up this spec is getting, but that edit is out of this pass's scope (CLAUDE.md was not touched here).
- **Checked and confirmed OUT OF SCOPE, no edit needed:** `.claude/skills/startup/SKILL.md` was updated 2026-07-09 (PR #564) to default to heavy Agent-subagent delegation with explicit Haiku/Sonnet/Opus model-tier labeling for the rest of a session. That's a working-process/tooling convention, not part of "the visual language of Rental Wrangler and the machinery that enforces it" (§1.1's stated scope for this area) — it doesn't touch tokens, the R-rulebook, the flag-color engine, the Rulebook overlay, `DESIGN.md`, or the CI drift guards. No change made here on its account.
- Everything else audited in this pass (§2.1 tokens, §2.3 flag-color engine, §2.4 Rulebook overlay + Windows catalog, §2.5 `DESIGN.md`, §2.6 the three CI drift guards, §2.7 the `jactec-ui` skill, §3 the tier ladder / gate model) is **still accurate as documented** — no further status markers needed beyond the R-rulebook ceiling correction above.
- **[2026-07-11] SHIPPED — toolbar reorg + list-view group-label pass.** The bottom bar's dozen-plus bare icons (QR, hover-previews, hotkeys, the 6 GPS fleet tools, dev/admin-unlocked tools) now collapse into two menus reusing the existing `openDropdown`/`.dd-item` pattern: a 🔔 **Comms bell** (Requests, Transport alerts, Notifications — rolled-up badge counts) and a 🔧 **Tools wrench** (`toolsBtn`/`toolsMenuRows`/`openToolsMenu`); the Comms bell is now the last element in both the desktop `bb-utils` row and the phone top-toolbar. "Round-Up" was renamed **Reports** and moved out of the header face into the logo menu (`openLogoMenu`). Separately, the shared group-header pattern (`.grp-hd`/`.grp-label`, `appendGroupedSections()`) got its label text bumped 9.1px→11px (+20%, landing on the existing 11px badge rung) and its trailing divider changed dashed→solid; a new left-side vertical rail (thickened `border-left` 1px→2px) was added to stacked-row grouped lists (Customers) but deliberately **not** to card-grid grouped lists (Units/Rentals) — a per-tile rail there would read as noise across wrapped grid columns. No new `data-r`-eligible element types were introduced (toolbar `iconbtn`/`dd-item` rows were never part of the pill/flag rulebook family; the label/line/rail changes are style-only on already-stamped elements).
...
| R28 | Account button | `acctBtn` | The stamped button on the customer funnel gate row — label = the account TYPE (Contractor/Business/Member…); opens the agreements window (same `js-view-agreement` access as the signed-agreement pill). Neutral steel chip, not an ignition/status color. (Customer Details reorg, **SHIPPED 2026-07-09**.) |
| R29 | Invoice action menu | `invoiceStatMenu` | The expanded-invoice header control: a hazard-stripe status pill (green solid = paid · yellow-stripe = partial · red-stripe = due; goes SOLID while its menu is open) that DOUBLES as the Pay · Print · Send · Refund action menu. A pressable-status control like R1, but it opens actions rather than advancing a status; Pay/Refund reuse the `canMoney()`-gated payment window. (Customer Details reorg, **SHIPPED 2026-07-09**.) |
| R30 | Paused banner | `.wr-paused` (`wranglerDockBodyHtml`) | Red hazard-stripe plate inside the Mr. Wrangler dock/rail window — raised when a Developer-tier operator takes the wheel (Wrangler Ops live jump-in, §18i); the composer goes read-only until released. (Wrangler Ops, **SHIPPED 2026-07-09**.) |
| R31 | Toggle chip | `toggleChip` | A single interactive on/off pill (PO required, Rental Protection, Member-Mode) — off = quiet outline, on = the registry tone color fill (`.chip-toggle.on.tone-*`). Distinct from R14: ONE control, not a joined group of options. (Account/Agreements redesign, **SHIPPED 2026-07-11** to staging.) |

> **Phase 6 — colored-dot → background-tint sweep (SHIPPED 2026-07-11 to staging, PR #588 → `area/design-system`).** Per spec §7c: the two genuine 3-state red/yellow/green **status** toggles now read their status via a background tint on the control itself instead of a corner dot — the customer funnel toggle's next-action urgency (`.funnel-sec .seg button.due/soon/ok`, `funnelSectionHtml`) and the card-capture signing-progress tabs (`.ag-tab.ok/mid/bad`). Both use the existing soft-bg + solid-ink convention (`--red-bg`/`--yellow-bg`/`--green-bg`, matching `.c-green`/`.c-yellow`/`.c-red`); the selected funnel segment stays solid orange (rule #3) with a thin inset ring for `due` so urgency survives on the active tab. Scoped deliberately: the four 6-color **category** pickers (chat-rail tabs, chat-member toggles, note color-tag picker, comment-flag picker) were left on dots — they encode category, not status.

`RULE_META` (`app.js:4351–4383`) is the machine-readable mirror of this table; the in-app Rulebook + Design Inspector render from it. **Note [UPDATED 2026-07-09]:** the roadmap/CLAUDE.md still say "R0–R24"; the live `RULE_META` now carries through **R30** — R25 sync banner and R26/R27 (manual-link, Due-Today banner) were already truthed-up here 2026-07-08; **R28/R29 (Customer Details reorg) and R30 (Wrangler Ops) shipped 2026-07-09** and are new to this table as of this pass. This drift between docs and code is logged as an Open Question (§11 Q1) — its recurrence three rules deeper within a day of the prior truth-up is itself evidence for that question, not just a hypothetical. Whether R25+ "count" as per-element stamps is the same open debate.
...
| Doc/code rule-count agreement (R24 vs R25 vs live R30) | Drift — docs lag code, and the gap widened again 2026-07-09 (R28/R29/R30 shipped; CLAUDE.md/roadmap still say R24). |
...
12. **Doc/code agreement (Phase 1):** the rule-count claimed in CLAUDE.md/roadmap/`DESIGN.md` equals the live `RULE_META` ceiling (**as of 2026-07-09, live ceiling is R30** — R28/R29 Customer Details reorg, R30 Wrangler Ops; this AC's own "today R25" was already stale from the 2026-07-08 R26/R27 truth-up and is now two rounds behind; CLAUDE.md/roadmap still say R24 — Q1).
...
1. **Rule ceiling drift — R24 or R25?** *(Status note, 2026-07-09: overtaken by events — live `RULE_META` is now at **R30** (R26/R27 landed 2026-07-08, R28/R29/R30 landed 2026-07-09), and CLAUDE.md/the roadmap still haven't moved off "R0–R24." The drift this question worries about isn't hypothetical; it's compounding roughly one truth-up's worth every day or two. That argues for the Q9 CI cross-check more than for another one-off manual truth-up.)* Live `RULE_META` includes **R25** (sync banner) but CLAUDE.md, the roadmap, `rulebook.md`, and likely `DESIGN.md` still say "R0–R24." *Decision:* truth-up the docs to R25 (and add a guard so the count can't drift again), or is R25 considered "not a stamped element" and excluded from the count? **Trade-off:** consistency vs. the semantic argument that the sync banner isn't a per-element rule.
