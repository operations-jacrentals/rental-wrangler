...
**Defaults adopted:** OQ-2 → `backendVersion` + capability negotiation · OQ-3 → CI lint asserting `PERSIST_KEYS`/`PERSIST_ID`/`IDX_MAP` key-parity · OQ-5 → per-entity sync applied/rejected counts · OQ-14 → **Stripe idempotency keys** on `stripeChargeInvoice`/`recordManualPayment` · OQ-12 → fix the `wrangler-needs-jac` label drift via `/clasp` (Phase 1) · OQ-6 → pricing floors stay **client-hide** (per `units-fleet` D1; no server-redaction) · OQ-11 → record-level last-clean-writer-wins + invoice action-locks (no field-level merge) · OQ-9 → no always-on sync pulse (silence = saved) — **reversed 2026-09-25 (owner, RC-67 2A): an always-on R37 freshness line, see §6.2** · OQ-10/OQ-4 → admin Health plate + `health` probe deferred (optional) · OQ-8 → server audit log deferred (revisit with compliance) · OQ-15 → durable offline queue is `frontend-performance` scope.
...
- **RC-67 3A (2026-09-25):** also refreshes at once on return to screen (`visibilitychange`→visible, window `focus`, `online`, and a back/forward-cache `pageshow`) — one load per return (a refresh that began < 10 s ago covers it, except after `online`), and a load that starts restarts the 18 s tick behind it so the two never stack. A lost `load` (stall, drop, echo 404) no longer skips the chats + rail pull, and the FIRST loss in a row is retried once ~3 s later (a second loss waits for the tick). A real sign-in refusal still skips chats + rail and is never retried. `window.__poll()` reports this tab's loads tried / lost (console only, no personal data).
...
> **Superseded 2026-09-25 (owner, RC-67 2A).** Not this pulse: an always-on **R37 freshness line** under the header name row — "Updated N s ago", becoming "Catching up…" after 60 s without a successful refresh (a backend load this device *applied*). It answers "is my screen current?" (reads); R25 still owns "not saving" (writes).
