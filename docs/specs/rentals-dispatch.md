...
> **[2026-07-11] FIXED — unit-swap billing-credit gap (real customer bug, Kerrigan).** `unitBilledRental(inv, r, unitId)` (`app.js:975`) now takes the rental object, not just its id. On a **single-unit** rental it credits any `rental`/`extension` line for that rental regardless of `unitId` — a unit SWAP (remove old + add new) money-safely keeps the old unit's PAID line in place (refund-first, §7.4), but it was tagged to a unit no longer on the rental, so every later extension/credit calc saw it as unbilled and re-billed the full amount. A genuine multi-unit rental still matches strictly per-unit (locked by logic-test `r5`). Two related gaps fixed alongside it: `inv-line-remove` (`app.js:17071`) now only detaches a rental from an invoice when no OTHER line still bills it, instead of unconditionally (a second rental-kind line on the same invoice — e.g. a since-removed manual add — used to sever the link even with a paid line still present); and `createInvoiceForRental` (`app.js:19452`) now refuses to mint a second invoice when `rentalInvoices(r)` already finds one linked (logic-test `r6`), closing the path that produced a duplicate full-price invoice after an unlink.
