...
| 262 | **THE ARTWORK BUDGET EXISTS AND HAS THREE SEPARATE DENOMINATORS — 900 KB wire / ≤16 MB decoded / ~5 ms frame. `docs/design/ART-BUDGET.md`, 2026-08-09.** ⏳ **PROPOSED — §7 delivery call is Jac's.** Closes ledger open-question (A) ("there is NO byte budget anywhere") and (D) (raster-vs-vector has no threshold). Measured, not reasoned: the running app booted headless at `#local`, the shipped payload `curl`ed off the live host, formats re-encoded from this project's own art, browser ceilings read from Blink/WebKit source and adversarially re-verified — **9 of 14 checked numeric claims were corrected or refuted**, including two (the "100 MB mobile Safari crash" unit error and the "w×h×4 always" decode formula) that would have skewed the budget by more than an order of magnitude in opposite directions. **The three denominators do not share a multiplier and confusing them is how every "it's only 40 KB" argument goes wrong:** wire bytes scale with the number of UNIQUE assets (Blink's `StyleImageCache` = one `ImageResourceContent` per resolved URL, so 96 elements sharing one `:root` asset pay once — this is why `#257` wins); decoded memory scales with the number of DISTINCT RASTERIZED SIZES (the decode-cache key is `{image, mip level, filter quality, colour space}` with no element identity, so one panel drawn at five widths costs five entries — which retroactively makes `#258`'s FIXED PANEL HEIGHTS a performance decision, not only a fidelity one); frame time scales with INSTANCE COUNT × per-element paint ops, and is the only one the shared-asset pattern does nothing for. **Only ROWS multiply** — measured census: card frames 3 (7 on phone), card headers 0 in list mode, group headers 10 (23 fixed, unbounded on Trips), footer 1, graph frames **0 until opened**, but rows are 60 at first paint (`VIRT_CAP`) +200 per Show-more, at 65.2 DOM nodes / 4.82 KB heap / 3.7 promoting elements each, taking full-document reflow from 9.0 ms to 103.7 ms at 269 rows while only 11.5% of those rows are ever on screen. So expensive technique is affordable everywhere EXCEPT rows: **`stretch` only above 24 instances** (`round` routes to `DrawImageTiled` plus a per-tile rescale), **≤1 mask layer per row** (mask-image is the slowest of 11 measured techniques at ~0.149 ms/element — 60 rows ≈ 8.9 ms, the whole 90 Hz frame, before the phone's 4.4× multiplier), **animate `transform`/`opacity` only**, no `backdrop-filter` on a repeated surface, no stylesheet `will-change`. `#254`'s 60→48 fps is exactly this arithmetic: +4.17 ms over 96 instances ≈ 43 µs per instance per frame. **The SVG-vs-raster threshold is ~26 PATHS** (measured 187 B/path gzipped) — and it is tighter here than elsewhere because **GitHub Pages serves gzip ONLY, brotli is not offered** (`Accept-Encoding: br` returns the file uncompressed), so a complex SVG costs 168 KB where a brotli host charges 37.6 KB. Coordinate precision 3dp→1dp is the only SVG lever worth pulling (−28.9% gzip; whitespace minification is −0.01%). **Percent-encode SVG data-URIs, never base64** (+0.24% vs +431% gzipped). Formats measured on this project's own art: WebP lossless for undithered flat panels (0.31× PNG), WebP q90 for exported panel art (4.9 KB vs 210 KB PNG), AVIF for photographic grain from a LOSSLESS master (−37% at matched SSIM), and **never lossy WebP for a mask** — it discards RGB under transparent alpha at PSNR 15.5 dB, which silently destroys the mask+tint pipeline. Dither is the byte trap: ±1 LSB grew a clean panel PNG **111×**. **THE BUDGET IS NEGATIVE TODAY.** First load is **6.02 MB wire, 83% of it media and none of it artwork** — `login-intro.mp4` 2.5 MB fetched `preload="auto"` before auth, `jac-rentals-logo.jpg` 1.65 MB at 4800² displayed at 62–82 px, `tex-metal-blued.jpg` 798 KB tiled at three different `background-size` values. Fixing those three frees ≈4.9 MB, five times the whole proposed envelope. Found in passing and independently actionable: **`sw.js`'s `SHELL` omits four modules `app.js` statically imports, so the offline shell cannot actually boot offline**; `?v=` provably cannot reach `assets/` (`cachebust.mjs` versions exactly three files), so asset versions must live in the FILENAME; and `vendor/plot.min.js` is 134 KB wire at boot for charts that are closed by default.  **AUDIT ADDENDUM, 2026-08-09 (§6.1–6.3) — answers “do I need texture assets?” with: mostly no, and you are already carrying two you did not know about.** All 13 exported assets in `docs/design/v2-card/` were decoded and inspected. Eleven are pure vector. **`asm-rowboard-image` and `asm-deck-image` each embed a raster PNG inside the SVG — 330×97 (33,573 B, **97.1%** of that asset) and 316×88 (28,839 B, **66.1%**) — and it is not grain, it is the three SLOT TICKS plus the BOARD SCREEN, baked as pixels.** That cluster has to come out for three reasons that are not about bytes: `#250` (slots are independent and individually clickable — a pixel in a background image cannot carry hover/focus/press or a hit target), `#238`/`#248` (the tick colours fail the CVD floor at 31–72 vs 90, unfixable in baked pixels, and it is a gate not a preference), and `#260` (the baked content is what pins rowboard's 500px left slice band). Extraction takes rowboard **46 KB → ~1.4 KB** and deck **58 KB → ~20 KB** — one authoring fix, four problems. The 77-byte 4×12 tile in `asm-headboard-image`'s `<pattern>` (the 3px scanline) is the RIGHT shape and should be the model. **Separately: the machined grain did NOT survive SVG export.** `#223` records the steel body as a solid plus a Figma SHADER paint (opacity .02, blend LIGHTEN) that IS the grain; SVG has no shader primitive, and verification shows `asm-housing-image` (33 paths) and `elbow-steel-image` (32 paths) carry **no embedded raster, no `<pattern>`, no `<filter>`** — on the two surfaces that repeat per row. The tonal sculpt survived (those paths ARE the bevels, trap 4); the fine grain did not. **So the ONE legitimately earned texture asset is a steel-grain overlay: one seamless tile, ≤256², lossless WebP, ONE `background-size`, declared once, ~2–5 KB** — a ~2% overlay, not a picture. Never a photographic metal plate for this job; `tex-metal-blued.jpg` spends 798 KB at 1024² on exactly that effect. That is ~5% of the texture allocation and the rest should stay unspent — the exported art carries its own tone and a second texture layer pays twice. **And the 798 KB already being paid is legacy, not part of the redesign:** `tex-metal-blued.jpg` binds to `bluedsteel` on `body`, `.col > .card`, `.card.anchored` and `.searchwrap`; three of those four are exactly what V2 replaces, so landing the artwork should RETIRE it rather than ship alongside it. New checklist item as a result: **decode every export and grep it for `<image`** — an SVG that embeds a raster is not a vector asset. | ✅ **§7 DELIVERY LOCKED — Jac, 2026-08-09: separate files under `assets/halo/`, version in the FILENAME, added to `sw.js` SHELL.** Budget figures themselves remain proposed. |
