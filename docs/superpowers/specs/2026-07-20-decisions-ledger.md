...
| 261 | **THE CONDUIT RAIL IS NEUTRAL STEEL AND NEVER TINTS — `#252` WINS, and this REVERSES the "tint per state at runtime" clause of `#259`.** | Jac, 2026-08-05, choosing option (A) when the conflict was put to him. `#252` and `#259` were both written the same day and contradict each other: `#252` says *"No moving current on the channel, elbow tint is right"* and that `--asm-channel` is a static background that never reads `--row-hue`; `#259` closed with *"author the rail NEUTRAL and tint per state at runtime, so one asset serves every state."* **`#252` is operative.** The channel is the shared trunk several elbows hang off, so it stays neutral steel; the **elbow** is the branch point to one specific row and is where that row's signal surfaces (`#217` — the laser follows the signal, the body never does). Build consequence: the rail ships as **one static asset with no mask and no `--row-hue` reader**, which is strictly less work than `#259` implied. The three accent stripes in `280:91` (`red — core`, `red — hot`, `red — well`) are flagged in Figma for recolour to neutral steel; the fills were left unchanged because the redraw is Jac's. |
| 262 | **THREE OF `#259`'s RAIL FACTS ARE WRONG — they were measured off a PNG, and the vector source contradicts two of the three "defects".** Read before redrawing the rail. Checked 2026-08-05 by dumping `280:91`'s children directly (13 full-height rectangles, exact x/width/fill). ① **The tile is 39px wide, not 41.** The stripes butt at exact coordinates and sum to exactly **39.000**, fully covering a 39px frame. ② **The "1px fully-transparent slot at x=19" does not exist.** There are zero gaps anywhere in the profile and every stripe is alpha 1.0. x=18.44 is the `core`/`hot` boundary and both are the same `#ff4242`, so it is invisible in the source; the PNG also reported `#9d0000` at x=18 where the vectors have `#ff4242`, which is what marks the raster as the unreliable measurement. ③ **The "right edge clipped mid-ramp" does not exist.** `absoluteRenderBounds` equals `absoluteBoundingBox` at w=39 — painted extent equals frame bounds, nothing is cut off. The far edge is a deliberate 1.41px `#33373b` stripe; wanting more falloff there is a design choice, not a bug fix. **What DOES hold from `#259`:** the red accent is real and off-canon, and the repeat-y tile decision is confirmed — all 13 stripes are full-height with zero vertical variation, so the panel is a pure vertical extrusion and any integer tile height seams perfectly. This is rule 1 of the pipeline (*the exported asset is ground truth*) earning its keep: a raster measurement invented two redraws that were never needed. |
| 263 | **BAKED TEXT CAN BE STRIPPED AT EXPORT TIME — a `TEXT` node does not require an art re-export, and `asm-headboard`'s "blocking" re-export in SLICE-SPEC §4 is CLEARED.** Measured 2026-08-05 on the timed pipeline run. SLICE-SPEC §4 listed the headboard's text-free plate as **blocking** (*"the corrected numbers do not pass without it"*) because "FAILED" sits dead centre in the stretch zone. It turned out to be a real `TEXT` node (`38:13`), so setting `visible = false` immediately before `exportAsync({format:'SVG_STRING'})` and restoring it after produces the text-free plate with **no redraw and no change to the Figma artwork**. The measured `43 47 59 30 fill` / `stretch round` bands then pass as written, with the group name supplied by live DOM. **Operative rule: before scheduling an art re-export to remove baked text, check whether the text is a `TEXT` node — if it is, the export strips it and the redraw is unnecessary.** This does NOT rescue text that has been flattened into a vector or an image fill, and it does not change `#260`'s verdict on the deck or the housing, whose blockers are geometric rather than textual. Whether it also clears `asm-rowboard`'s "PROMISED" is untested and open — if it does, `sliceLeft` should fall from 500 toward ~48 and the 572px floor with it, but the new bands would need measuring and adversarially re-rendering before the panel is shippable. |
| 264 | **THE FIGMA FILE CARRIES THE SLICE CONTRACT — hidden `SLICE GUARD` frames and component descriptions, so the numbers cannot silently drift from the art.** Jac, 2026-08-05: *"Whatever you have to do to make it work should also be done back to Figma so that they're in sync."* Implements SLICE-SPEC §6.A's *"give each of these a named spacer frame in Figma so it cannot be closed up by accident."* Applied to `38:8`: the baked text is renamed **`readout — LIVE DOM · EXCLUDE FROM ART EXPORT`**; a **hidden** child frame `SLICE GUARD — 43/47/59/30 · stretch round · do not decorate` marks the four cut lines and the three corridors; and the component **description** carries the slice numbers, the floor, and the four authoring rules. `280:91` carries the corrected tile contract from `#262`. **Hidden layers are absent from `exportAsync`** — verified in the same call that wrote them (`contains guard markers = false`), which is what makes the guard free: it travels with the component, is visible while drawing, and never reaches the asset. Toggle it visible to draw, hidden to export. Nothing rendered changed and no fills were edited, so instances are untouched. |
| 265 | **THE ART PIPELINE, TIMED END TO END: 15 min 27 s for one panel.** `asm-headboard` (`38:8`) from Figma export to published interactive artifact on 2026-08-05, against a 30-minute budget — the measurement `#257` was written to enable, after one card cost hours of rework. Rough split: ~4 min locating the node and reading its layer stack, ~2 min export and split, ~5 min CSS and artifact, ~4 min render-verify and two fixes. **The pipeline is not the expensive part — measuring and verifying slice bands is.** Today cost 15 minutes *because SLICE-SPEC already held the numbers*; a redraw invalidates its bands and the adversarial re-measure is the multi-hour job. Projection for the rest of the assembly: rowboard and channel ~10 min each, elbow and cap ~8 min each (fixed-size, export + tint mask only, no slicing), deck and housing ~15 min each **after** Jac's redraws — roughly an hour for everything ready today. The second panel onward is cheaper than the first because the CSS pattern, the inline-assets build step and the headless render harness are all reusable. Two fixes that only a render caught, both self-inflicted and both worth repeating as checks: a 14% state wash read as a red panel rather than tinted glass (dropped to 7%; the steel must not tint), and the naive "scaled" control was **letterboxing rather than smearing**, because an exported SVG keeps its aspect ratio under `background-size: 100% 100%` — a `preserveAspectRatio="none"` variant is required or the comparison is rigged in the method's favour. |
| 266 | **THE MESSAGE BOARD IS A MARQUEE — long text scrolls through the screen aperture, which settles its art contract.** Jac, 2026-08-05: *"ultimately the Message board is a marquee that hoses longer texts."* Two consequences, neither optional: **(a)** the label is **live DOM, never baked art** — the string changes per message and moves, so `#263`'s export-time strip is mandatory here rather than merely convenient; and **(b)** the screen interior must be a **clean uniform horizontal corridor** with no feature for scrolling glyphs to collide with or scroll behind, which constrains the redraw more tightly than a static panel would. Confirmed the strip works: `label` is a real `TEXT` node (`354:44`), and hiding it before `exportAsync` yields an SVG that does not contain `"PROMISED"`. That text is only **4.4 KB of the 664 KB export** — it was never the expensive part. Marquee mechanics (viewport `overflow:hidden`, the scroll transform, whether short strings should sit static rather than crawl) are **not yet decided** and are separate from the art contract recorded here. |
| 267 | **`asm-rowboard`'s 572px FLOOR IS CAUSED BY A DUPLICATED, MISPLACED SIGNAL ROW — not by the baked text, and `sliceLeft: 500` should never be trusted again.** Found 2026-08-05 by walking `354:38`'s tree with render bounds and effects. Inside the `glow` frame sit **TWO identical `Slots — signal row` instances stacked on each other** (one is a straight duplicate), each holding 3 `Bulb` instances at **120 vector paths apiece — 720 paths total**, which is the entire ~664 KB export weight. Both sit at roughly **x −189 → 123** against a 470px frame: hanging 189px off the left edge while overlapping the board's left end. So `absoluteRenderBounds` is **657 wide against a 470 frame (dx −187)** — trap #2, *painted extent is not frame bounds*, firing on a second component. Any horizontal cut line must clear a feature at x=123, **and** numbers measured off that export are shifted by 187px. That is the mechanism behind `sliceLeft: 500` and the 572px floor. **The fix is a decision only Jac can make** — (A) the bulbs are strays, delete both, cuts fall to roughly `20 / 32 / 16 / 24` against the bezel for a floor near **56×36px** and a few-KB export; or (B) they belong, de-duplicate to one row moved fully inside the frame, `sliceLeft` clears its right edge (~130) for a floor near **162px**. Either way the 572px floor is wrong. Measured geometry recorded in the component description for whichever way it goes. **General lesson: when a slice number looks absurd, walk the render bounds before believing it — twice now the absurd number was an artifact, not a constraint (`#262`, this row).** |
| 268 | **SLOTS ARE THEIR OWN ASSEMBLY MEMBER — they compose ALONGSIDE a board, never baked inside one. The stray rows in `asm-rowboard` are now hidden and the panel is unblocked.** Jac, 2026-08-05: *"bulbs (slots) are their own thing."* The assembly taxonomy already said so — deck, housing, conduit rail, elbows, boards, **slots** are peers — so the `#267` (A)/(B) question was never open; slot artwork inside the Message Board's `glow` frame is contamination by definition. **Resolved by hiding, not deleting** — the two duplicate `Slots — signal row` instances remain in the file one toggle from restoration, because they are Jac's artwork and a stacked duplicate might yet be deliberate; hidden layers are absent from `exportAsync`, so the asset is clean either way. **Measured effect, all five variants:** `absoluteRenderBounds` 659 → 470 with **dx −189 → 0** (painted extent now equals frame bounds exactly), and the SVG export **652.7 KB → 3.9 KB, 99.4% smaller**, as 720 bulb vector paths leave the asset. For contrast the baked text was 4.4 KB of the old 664 KB — the text was never the cost. **Generalised rule: before slicing any panel, check its tree for members of OTHER assembly components. A peer component nested inside another's art inflates the export, breaks render bounds, and invents slice constraints that do not exist.** |
| 269 | **`asm-rowboard` CANDIDATE CUT LINES: `18 32 15 22 fill` / `stretch round` — DERIVED FROM GEOMETRY, NOT YET RENDER-VERIFIED. Do not copy into SLICE-SPEC.md until a resize render confirms them.** Computed 2026-08-05 from `354:38` (470×97) once `#268` cleared the strays. The innermost surviving feature is `ring core` at inset L=17.2 T=12.95 R=27.63 B=10.33; the cuts clear it plus its stroke so the centre region is pure screen. Implied floor ≈ **54×33px against the old 572px** — a ~10× improvement, and the second time today an absurd slice number proved to be an artifact rather than a constraint (`#262`, `#267`). The screen aperture is x 17.34–442.23, y 13.04–86.58 — **that rectangle is the marquee viewport** (`#266`), so the centre must stretch to it cleanly and stay free of decoration. **These numbers are explicitly provisional.** The measure-and-adversarially-verify step is the expensive half of the pipeline (`#265`) and has NOT been run here; `#262` and `#267` are both cautionary tales about trusting an unverified measurement. A render pass across several widths, checking the corridor does not smear and the scanline rhythm stays phase-closed under `round`, is required before these ship. Recorded in the Figma component description and in a hidden `SLICE GUARD` frame on all five variants, both labelled CANDIDATE. |
| 270 | **`asm-rowboard` SLICE VERIFIED AND PROMOTED: `0 66 0 56 fill` / `stretch`, height LOCKED 97px, floor 122px (240px practical).** Rendered 2026-08-05 at 122/180/240/380/470/700/1100/1600px against a `preserveAspectRatio="none"` naive-scale control; chamfers hold 45° and the ring stroke holds constant thickness at every width, while the control rakes the chamfers flat and balloons the stroke by 1100px. Now in `SLICE-SPEC.md`, replacing the old `684×97 / 0 72 0 500 / min 572px` — **those numbers came from the contaminated export** (`#267`: variant `21:19`'s render width with the strays was 683.4, which is the 684). **`#269`'s candidate `18 32 15 22` WAS WRONG and the render is what caught it.** Both 45° chamfers span y 5.77–44.44 and 56.89–92.94, so containing them in border-image corners needs T≥44.4 and B≥40.1 of a 97px height — no uniform horizontal band exists, and those cuts would have sliced straight through both. **The vertical slice must be 0 and the height is genuinely locked**, exactly as SLICE-SPEC line 247 said before I second-guessed it. Horizontal cuts are set by the innermost feature `ring core` (bottom-left chamfer ends x=49.85, top-right begins x=409.72) plus its 2.8 stroke and 0.81σ blur → L≥53.7 / R≥64.1, rounded to 56/66. **`stretch` is provably lossless here:** the scanline tile decodes to 4×12px with **every row a single constant colour across all four columns** — pure horizontal banding, so no horizontal information exists to smear, and `round` would only risk a partial tile at the seam for no gain. Vertically the 3px pitch never rescales because the slice is 0. **Method note: this is the third artifact-driven wrong number today (`#262`, `#267`, and my own `#269`) — and the only one caught by rendering rather than by re-reading geometry. Rule 2 of the pipeline is load-bearing; derive candidates from geometry, but never promote a band that has not been rendered and looked at.** |
| 271 | **`asm-channel` VERIFIED AND PROMOTED: a `39×4` `repeat-y` tile, seamless at every height and DPR tested. Not a 9-slice, and 39px wide — not 41.** Run 2026-08-05, ~8 minutes. The component (`280:91`, 39×140, 875 bytes) is **clean** — zero nested components, zero TEXT nodes, zero render overhang — so the stray-sweep and render-bounds checks that `#267` added to the method cost nothing here and confirmed it outright. **Verification:** rendered at heights 40 / 97 / 140 / **333.5** / **777.25** px × DPR **1 / 2 / 3** — fractional heights deliberately included, since that is exactly where `repeat-y` resampling seams. **All 45 combinations clean: zero seam rows, max row-to-row delta 0.** Then, because a *missing* asset would also read as vertically uniform, the columns were checked against the expected 13-stripe profile and matched **13/13 byte-exact** — the clean result is real, not a false pass on transparency. `#262`'s corrections all confirmed against the vector source: 13 stripes butting x=0 → exactly 39.000, no gaps, all alpha 1.0. **On the 4px tile height:** nothing in the art needs it (zero vertical variation means a 1px tile carries identical information), it is insurance against fractional-DPR edge sampling, and it measured identically — so it costs nothing and stays. **The accent stripes are still red** (`#ff4242`, `#9d0000`) pending Jac's redraw per `#261`; the tile geometry is colour-independent, so recolouring changes no number here. |
| 272 | **OPEN FOR JAC — the conduit rail could ship as a pure CSS gradient with no asset at all, and it measures pixel-identical. Flagged, NOT taken, because it would bend art-pipeline rule 1.** Because the rail is 13 solid vertical bands and nothing else, `linear-gradient(90deg, …)` with hard stops reproduces it **exactly** — measured in the same harness as `#271`: 13/13 stripes byte-exact, zero seams, at all five heights and all three DPRs. It removes the image decode entirely and makes recolouring (`#261`'s neutral-steel redraw) a token swap rather than a re-export. **Rule 1 of `art-pipeline` says export art rather than recreating it in CSS**, and that rule is well-earned — hand-recreation is normally lossy and expensive. This is the degenerate case where the recreation is provably exact rather than approximate, which is a materially different situation, but **canon changes are Jac's call and a measured exception is still an exception.** The shipped form in SLICE-SPEC remains the exported `39×4` tile; the generated gradient is kept at `docs/design/halo-elements/assets/asm-channel-gradient.css` so the choice stays cheap either way. **If this is accepted it should be scoped narrowly — "art that is provably N solid rectangles and nothing else" — not loosened into general permission to rebuild art in CSS.** |
| 273 | **`asm-elbow` VERIFIED: fixed `224×157`, FOUR stacked layers (plate-under · well mask · signal mask · plate-over), recomposite is visually lossless.** Run 2026-08-05. The elbow is the part that carries the row signal — 49 paths, of which **7 are accent** (`#FF4242`×5, `#9D0000`×2) — which independently confirms `#252`/`#261` from the art itself. **Two masks, not one:** the accent is two-tone, and luminance-encoding the dark tone into a single mask is impossible because a tint composited over any base can never reach `#9D0000`'s zero green/blue; two masks also keep "how does the well behave per state" an open token rather than a baked choice. **The plate is split in two because path order interleaves steel and accent** — `steel(0,1) · well(2) · steel(3) · bright(4,5,6) · steel(7–24) · well(25) · bright(26,27) · steel(28–48)`. Bright paints above well in both clusters, and steel occludes accent at several indices; collapsing all steel under all accent produced a **visible wrong-colour block of 1,816 px** reading `#9d0000` where the source is `#151618`. **Measured fidelity of the final split: 0.791% of pixels differ at all, max channel delta 36, only 0.031% differ by more than 16** — all of it anti-aliasing at shared edges, which is inherent to splitting paths across compositing layers and is a *compare-by-distance* result per rule 1. Path count 2+2+5+40 = 49, so nothing was dropped. **CSS gotcha worth keeping: use explicit child elements, never `::before`/`::after` for layer stacks** — pseudo-elements stack as first and last child, so a real child lands BETWEEN them, which silently put the signal mask above the top plate on the first attempt. |
| 274 | **`asm-cap` VERIFIED: fixed `39×22`, ONE static asset, no mask — and its frame dimension is a trap.** Run 2026-08-05, the cheapest panel in the set. **The exported asset is `39×22` while the Figma frame reads `39×18.8`**: `clipsContent` is false and the rotated `cap — fitting` child overhangs the frame bottom by 3px, so Figma expands the export to contain it. **Size the cap at 22px — using the frame height squashes or clips the fitting.** The cap's 10 paths contain **zero accent colours** (only `#151618`, `#3D4146`, `#575D64`, `#7A828C`), so it needs no mask and stays neutral in every row state — a third independent confirmation of `#252` that came from the art rather than from the ledger. |
| 275 | **METHOD CORRECTION: the render-bounds overhang check is UNRELIABLE on a frame with `clipsContent = true` — it reports the clip rect and always reads "zero overhang".** Found 2026-08-05 on `asm-elbow`, whose component reported render bounds exactly equal to frame bounds while its children actually occupy only x 54–214, y 42–139. The check that caught `#267`'s 189px rowboard overhang only worked because that frame does not clip. **Correct method: check `absoluteRenderBounds` on the CHILDREN individually, and read `clipsContent` before trusting a parent-level result.** Two further export traps recorded the same day: **(a)** `download_assets` bakes a `<rect fill="#F5F5F5"/>` canvas background into the SVG that the plugin-API `exportAsync` does NOT — strip it or the art ships on an opaque light-grey backing; **(b)** `mask-image` is blocked by CORS from `file://`, so a local render harness silently drops every mask and reads as a failed split — **inline assets as data URIs**, which is the production form anyway. |
