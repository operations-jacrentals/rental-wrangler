...
| 262 | **A YES/NO SPEC FACT IS NOT A STATUS — its selected toggle cell takes the ONE orange, not Coverage's green/red.** | Settled 2026-08-20 building the unit card's Immobilizer field (#820). Coverage's `Insured`/`Uninsured` seg is green/red because uninsured genuinely IS a risk state; "no immobilizer" is not — most of the yard has none, so a red cell would paint a permanent false alarm across the fleet. `wrangler-style` §3 ("toggle active segment = the filled Signal chip of the option's status; falls back to **orange** when the option carries no status") and ledger `#16` already rule this; the agency law (`#137`, ACCENT ≠ STATUS) is the reason — orange says *you set this*, a status hue describes *the world*. **Test to apply next time:** does the option map onto a status registry (`gpsStatus`, `unitFleetStatus`, …)? Yes → the status hue. No → orange. Also settled with it: a detail sub-field of a yes/no fact renders when the fact is on **or** when it already holds content, so flipping the toggle off never eats what was typed (Coverage hides its riders unconditionally, which is the weaker half of that precedent). |
| 263 | **A ROLLUP OF RATIOS IS RE-DERIVED AT THE NEW SCOPE, NEVER SUMMED — and it rides the existing pinned `ru-real` column, not a new footer.** | Settled 2026-09-21 building the All-Fleet total on Dollar Utilization (#837). The approved plan said "sum the dollar-utilization column across all categories"; percentages over **different denominators** cannot be added — on the demo fleet the naive Σ reads **178%** where the true figure is **52%**. So the rollup re-runs the N11 formula at fleet scope (`Σrev annualized ÷ Σbasis`), which IS the cost-weighted total and therefore the one aggregate that reconciles with the bars above it. Two scope rules fall out and generalize to any future rollup: it is computed from the **full data rollup, not the drawn rows** (the bars stop at `.slice(0,12)`, an "All Fleet" number that silently covered 12 of 40 categories would be a lie), and **idle capital stays in the denominator** (a category with basis but no revenue is dropped from the bars yet kept in the divisor — unrented iron is precisely what a utilization number exists to expose). Placement: the plan asked for a bottom-pinned footer row "so it stays visible while the category list scrolls", but the panel is a fixed 12-bar SVG with no scrolling list and no table; the purpose (always visible, range-aware, reconciles) is met by `ruDuo(chart, ruReality(…))` — the board's **already-shipped** pinned-summary grammar used by Bookings · Inspections · Voided. Reusing it adds **zero CSS, zero new colours** (frozen palette, `#1`), keeps the readout off `--accent` (agency ≠ status, `#137`), and needs no `data-r` stamp (chart-engine output is outside the atom families). Do not improvise a new footer container for a summary a panel can already pin. | ✅ **LOCKED — 2026-09-21** |
