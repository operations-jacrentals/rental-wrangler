...
- **✓/✕ prefix, not prose (Jac, 2026-07-08):** entries that came from a completed/cancelled Next Action render with a **green ✓ / red ✕ prefix** on the log row instead of spelling out "Completed"/"Cancelled". Keeps rows terse and scannable; other log entries (payments, deliveries, system events) render without a prefix.
