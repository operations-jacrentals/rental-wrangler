...
A3. **New additive endpoint** `GET /api/fleet/status` in the backend (§5 of the spec): one call returning `{ deviceId, provider, online, ignition, lastSeen, lat, lng }[]` aggregated across all four providers for every mapped device. **Additive only** — it composes the existing per-provider status paths; it must not change any existing route. This is the one backend code change; it lives in the forked repo, deploys via Railway, and is NOT part of Rental Wrangler's git.
