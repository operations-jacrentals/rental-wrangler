...
- **Questions → hybrid (Jac, 2026-07-15).** Surface a decision in whatever form reads clearest: **formatted inline** for exploration/nuance (lead with the outcome, no bullet-blobs), a crisp **structured block** for a clean either/or, an **artifact** for anything comparative/visual, or an `AskUserQuestion` popup. Inline is favored; this supersedes the old popup-only rule.
