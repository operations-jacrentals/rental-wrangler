...
- **Backend deploy auth = check `GAS_SA_KEY_B64`, NOT clasp (Jac, 2026-07-06 — clasp's OAuth is currently broken).** Confirmed: even a **brand-new** clasp token, minted fresh in a **cloud** session, fails `invalid_grant / invalid_rapt` immediately — this is a Google Workspace re-auth policy on the `cloud-platform` scope, enforced server-side per call, and no amount of re-`clasp login`-ing fixes it. **Do not re-attempt `clasp login` as a troubleshooting step** — it will not work until the Workspace admin changes that policy.
  ...
  ```
  - **`GAS_SA_KEY_B64` set** → backend deploys are reachable via the **service-account path**: `docs/handoffs/gas-deploy-service-account.mjs` (push/deploy through the Apps Script REST API directly, JWT auth, not subject to RAPT). **Full runbook + the pending deploy queue: `docs/handoffs/BACKEND-DEPLOY-QUEUE.md` — this file lives on the `staging` branch, not `main`.** If you're on `main` and can't find it: `git fetch origin staging && git checkout staging` first (a session got stuck on exactly this on 2026-07-06 — don't repeat it). `/clasp` has the full detail; same STOP-gate before any actual `deploy` call.
...
- **Relaying to a PARALLEL session (Jac, 2026-07-06) — hand back a structured go, not a one-liner.** When Jac pastes a status update, screenshot, or a question **from a different running session** (he often runs several in parallel — a backend deploy, a build sprint, etc.), don't just answer inline — draft him a **copy-pasteable prompt for that other session** shaped as:
  1. **The explicit go/no-go** stated first, in plain words ("run the actual deploy now" / "hold — do X first").
  2. **Ordered steps** the other session should take, in the sequence that actually matters (pull-before-edit, diff-before-push, etc.).
  3. **Explicit STOP points** re-stated (e.g. "show me the diff before pushing," "confirm with me again before the live deploy call") — don't assume the other session remembers its own STOP-gates.
  4. **A verify step** — how it proves the action worked (a curl check, a screenshot, a log line), not just "done."
  5. **What to report back** so the loop closes cleanly.
  This worked well relaying a backend-deploy decision across sessions — reuse the shape any time Jac is bridging between two live chats rather than acting in just one.
