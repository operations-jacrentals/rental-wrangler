...
- **Promotion cadence — propose the hops, never auto-promote to live:** when a task is *done*, offer to merge `<domain>/<task>` → `area/<domain>` (the merged task branch then self-cleans via the branch janitor). When Jac wants to preview/QA, merge `area/<domain>` → `staging`. **Then TWO deploy steps are mandatory — skip either and staging serves a stale build (this has bitten us):**
  1. **Bump the shared `?v=` cache token** on `style.css` / `rule-usage.js` / `app.js` in `index.html` to a value *newer than staging's current* — other branches may already have bumped it, so check first: `git show origin/staging:index.html | grep '?v='`. (A same-token file swap stays cached ~10 min.)
  2. After pushing `staging`, **force the mirror sync now** — don't wait on its 10-min cron: `gh workflow run sync-staging.yml --repo operations-jacrentals/rental-wrangler-staging`. Then **verify the LIVE site serves the new bytes** (`gh run watch <id>` → `curl -s https://operations-jacrentals.github.io/rental-wrangler-staging/app.js | grep <a marker only the new code has>`). The mirror can serve an OLD file under a NEW token — a stale snapshot that **no browser refresh fixes**; only a re-sync does.
  - Then tell Jac to **hard-refresh** staging (Ctrl/Cmd+Shift+R). Only after he confirms it's clean there, open a PR `staging` → `main` (protected; CI). **`main` is live — promoting to it is always Jac's explicit call.**
## 5. Ready summary
