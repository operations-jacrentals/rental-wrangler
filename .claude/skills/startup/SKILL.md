...
- **The two-gate deploy loop** (build → review → integrate → wait → go-live) — both gates run on Jac's say-so:
  1. **Deploy to staging** — run the **`/deploy` skill** (wraps `node tools/deploy-staging.mjs`). By default it publishes a **Staging Deck** deploy — an immutable numbered folder `d/<id>/`, not a slot; open the stable **`…/rental-wrangler-staging/d/`** launcher (always redirects to the newest deploy) or the deploy id/label `/deploy` gives you, then use the in-app **`Staging ▾`** switcher (bottom-right, staging-only) to jump between recent deploys. (`--slots` falls back to the old 3-slot lease pool: slot 1 = `https://operations-jacrentals.github.io/rental-wrangler-staging/`, slots 2/3 = `…-staging-2`/`-3` — review the slot the deploy landed on; [[jactec-staging-url]].) The script **curl-verifies the live staging bytes and exits non-zero if staging didn't actually update** (a "pushed" is not a "served"). A failed or unverified deploy is a **HARD STOP** — see the working-rules bullet; never work around it.
...
- **Questions → popup-first, single-attempt (Jac, 2026-07-16).** Route **every** decision and question through the `AskUserQuestion` popup — try it **exactly once**. If that one popup fails, do **NOT** retry it; fall back to **inline**, presenting the same question and the same options as lettered choices (**A / B / C … + Other**) in a crisp structured block. This supersedes the 2026-07-15 hybrid rule. **Batch related questions into one popup (up to 4) and favor multiSelect when the answers aren't mutually exclusive** (Jac: *"I love the multiselect most"*). (Artifacts still *show* anything comparative/visual; the popup — or its inline fallback — is how you *ask*.)
...
- **Any new or reshaped UI → `style` + `wrangler-style`, ALWAYS together.** `wrangler-style` holds the
  **decisions** (the locked steel palette, the two type voices — Archivo body + mono stamped, the four
  control shapes, the Signal · Gate · Stamp · Ref · Door · Pin · Field vocabulary, the voice);
  `style` holds the **measurable rules** those decisions must satisfy (one control height, the size
  ladder, one shape per control family, WCAG floors, the ≥90 CVD separation, 60-30-10). When a
  decision and a rule conflict, **the decision moves, not the rule.** Backend (`Code.gs`), CI scripts
  and pure logic are exempt.
  *(`jactec-ui` was **deleted** 2026-07-26 — ledger #26/#91. It taught the superseded language:
  `#ff7a1a`, Saira Condensed, one 7px chip radius, hazard-stripe + rivets. Do not resurrect it. Four
  capabilities it uniquely held — the **`/role` audit**, **mobile** reflow/touch, **anti-slop**, and
  the **DESIGN.md** guides — are preserved as reference in
  `docs/design/reference/legacy-jactec-ui/`, and still need proper skill homes.)*
...
- **Specs:** after generating or changing a spec/feature/screen, offer to run the **`/role` audit** — the 15 role lenses + the authority / data-sensitivity / gate checklist that catches margin and PII leaks before building. It has no skill home right now; its canon is `docs/design/reference/legacy-jactec-ui/role-framework.md` + `role-roles.md`.
